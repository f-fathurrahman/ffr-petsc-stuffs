...
void Calculate_force(SDDFT_OBJ *pSddft) {
  ...
  PetscInt Xstart, Ystart, Zstart, Xend, Yend, Zend, XBJstart, YBJstart,
      ZBJstart, XBJend, YBJend, ZBJend;
  ...
  PetscScalar tableR[MAX_TABLE_SIZE], tableVps[MAX_TABLE_SIZE], Bval,
      noetot = pSddft->noetot;
  PetscScalar ***pVpsArray = NULL;
  PetscScalar ***pBArray = NULL;
  PetscScalar Dtemp;

  PetscInt i = 0, j, k, xi, yj, zk, offset, tablesize, l, Ii, J, K, p;
  PetscInt nzVps, nyVps, nxVps, nzBJ, nyBJ, nxBJ, nzGradBJ, nyGradBJ, nxGradBJ,
      i0, j0, k0, a, poscnt, index = 0, index_force = 0, index_mvatm = 0, count,
                             at;
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
     ...
     * ar r=0, we assume the pseudopotential value at r=0 to be same as the
     * first entry read from the file.
     */
    tableR[0] = 0.0;
    ...
    } while (PetscRealPart(tableR[count - 1]) <=
             PetscRealPart(1.732 * cutoffr));
    rmax = tableR[count - 1];
    int start, end;
    ...
    for (poscnt = start; poscnt <= end; poscnt++) {
      ...
      if (pmvAtmConstraint[index_mvatm] == 1 ||
          pmvAtmConstraint[index_mvatm + 1] == 1 ||
        ...
        /*
         * find if domain of influence of pseudocharge overlaps with the domain
         * stored by processor
         */
        ...
        if (overlap) {
          /*
           * find the starting and ending indices of finite difference nodes in
           * each direction for calculation of pseudopotential and pseudocharge
           * density
           */
            ...
            for (j = 0; j < nyVps; j++)
              for (i = 0; i < nxVps; i++) {
                ...
                if (r == tableR[0]) {
                  ...
                  ispline_gen(tableR, tableVps, count, &r, &pVpsArray[k][j][i],
                              &Dtemp, 1, YD);
                }
              }

          /*
           * calculate pseudocharge density at nodes inside the overlap region
           * from previously calculated pseudopotential values using finite
           * difference stencil
           */
          for (k = ZBJstart; k <= ZBJend; k++)
            for (j = YBJstart; j <= YBJend; j++)
              for (i = XBJstart; i <= XBJend; i++) {
                ...
                for (a = 1; a <= o; a++) {
                  Bval += (pVpsArray[K][J][Ii - a] + pVpsArray[K][J][Ii + a] +
                           pVpsArray[K][J - a][Ii] + pVpsArray[K][J + a][Ii] +
                           pVpsArray[K - a][J][Ii] + pVpsArray[K + a][J][Ii]) *
                          coeffs[a];
                }
              ...
              for (i = Xstart; i <= Xend; i++) {
                ...
                for (a = 1; a <= o; a++) {
                  if (pmvAtmConstraint[index_mvatm + 2] == 1)
                    GradBJz += (pBArray[K - o + a][J - o][Ii - o] -
                                pBArray[K - o - a][J - o][Ii - o]) *
                               coeffs_grad[a];
                  if (pmvAtmConstraint[index_mvatm + 1] == 1)
                    GradBJy += (pBArray[K - o][J - o + a][Ii - o] -
                                pBArray[K - o][J - o - a][Ii - o]) *
                               coeffs_grad[a];
                  if (pmvAtmConstraint[index_mvatm] == 1)
                    GradBJx += (pBArray[K - o][J - o][Ii - o + a] -
                                pBArray[K - o][J - o][Ii - o - a]) *
                               coeffs_grad[a];
                }
                PhimVpsJ = PotPhiArrGlbIdx[k][j][i] - pVpsArray[K][J][Ii];
          ...
          MatSetValue(ForceProcMatz, rank, poscnt, ForceProcComponentz,
                      INSERT_VALUES);
          MatSetValue(ForceProcMaty, rank, poscnt, ForceProcComponenty,
                      INSERT_VALUES);
          MatSetValue(ForceProcMatx, rank, poscnt, ForceProcComponentx,
                      INSERT_VALUES);
...
void Calculate_forceCorrection(SDDFT_OBJ *pSddft) {
  ...
  PetscInt Xstart, Ystart, Zstart, Xend, Yend, Zend, XBJstart, YBJstart,
      ZBJstart, XBJend, YBJend, ZBJend;
  PetscScalar x0, y0, z0, coeffs[MAX_ORDER + 1], coeffs_grad[MAX_ORDER + 1];
  PetscScalar GradBJx, GradBJy, GradBJz, GradBJx_TM, GradBJy_TM, GradBJz_TM,
      PhicmVtmJ, PhicpVpsJ, BtmpBps, FORCE_corr;
  ...
  PetscInt i = 0, j, k, xi, yj, zk, offset, tablesize, tablesize_TM, l, Ii, J,
           K;
  PetscInt p, nzVps, nyVps, nxVps, nzBJ, nyBJ, nxBJ, i0, j0, k0, a, poscnt,
      index = 0, index_force = 0, index_mvatm = 0, at, count;
  PetscScalar delta = pSddft->delta, x, y, z, r, cutoffr, rmax, rmax_TM,
              rcut = pSddft->REFERENCE_CUTOFF;
  PetscInt n_x = pSddft->numPoints_x;
  PetscInt n_y = pSddft->numPoints_y;
  PetscInt n_z = pSddft->numPoints_z;
  PetscScalar R_x = pSddft->range_x;
          ...
          for (k = 0; k < nzVps; k++)
            for (j = 0; j < nyVps; j++)
              for (i = 0; i < nxVps; i++) {
                ...
                pVpsArray_TM[k][j][i] =
                    PseudopotReference(r, rcut, -1.0 * pSddft->noe[at]);
              }

          /*
           * calculate pseudocharge density and contribution to self energy at
           * nodes inside the overlap region from previously calculated
           * pseudopotential values using finite difference stencil
           */
          for (k = ZBJstart; k <= ZBJend; k++)
            for (j = YBJstart; j <= YBJend; j++)
              for (i = XBJstart; i <= XBJend; i++) {
                ...
                Bval = pVpsArray[K][J][Ii] * coeffs[0];
                Bval_TM = pVpsArray_TM[K][J][Ii] * coeffs[0];
                for (a = 1; a <= o; a++) {
                  Bval += (pVpsArray[K][J][Ii - a] + pVpsArray[K][J][Ii + a] +
                           pVpsArray[K][J - a][Ii] + pVpsArray[K][J + a][Ii] +
                           pVpsArray[K - a][J][Ii] + pVpsArray[K + a][J][Ii]) *
                  ...
                  Bval_TM +=
                      (pVpsArray_TM[K][J][Ii - a] + pVpsArray_TM[K][J][Ii + a] +
                       pVpsArray_TM[K][J - a][Ii] + pVpsArray_TM[K][J + a][Ii] +
                       pVpsArray_TM[K - a][J][Ii] +
                       pVpsArray_TM[K + a][J][Ii]) *
                      coeffs[a];
                }
                pBArray[k - ZBJstart][j - YBJstart][i - XBJstart] = Bval;
                ...
                for (a = 1; a <= o; a++) {
                  ...
                  if (pmvAtmConstraint[index_mvatm + 2] == 1) {
                    ...
                    GradBJz += (pBArray[K - o + a][J - o][Ii - o] -
                                pBArray[K - o - a][J - o][Ii - o]) *
                               coeffs_grad[a];
                    GradBJz_TM += (pBArray_TM[K - o + a][J - o][Ii - o] -
                                   pBArray_TM[K - o - a][J - o][Ii - o]) *
                                  coeffs_grad[a];
                    GradVJz +=
                        (pVpsArray[K + a][J][Ii] - pVpsArray[K - a][J][Ii]) *
                        coeffs_grad[a];
                    GradVJz_TM += (pVpsArray_TM[K + a][J][Ii] -
                                   pVpsArray_TM[K - a][J][Ii]) *
                                  coeffs_grad[a];
                  }
                  ...
                  if (pmvAtmConstraint[index_mvatm + 1] == 1) {
                    ...
                    GradBJy += (pBArray[K - o][J - o + a][Ii - o] -
                                pBArray[K - o][J - o - a][Ii - o]) *
                               coeffs_grad[a];
                    GradBJy_TM += (pBArray_TM[K - o][J - o + a][Ii - o] -
                                   pBArray_TM[K - o][J - o - a][Ii - o]) *
                                  coeffs_grad[a];
                    GradVJy +=
                        (pVpsArray[K][J + a][Ii] - pVpsArray[K][J - a][Ii]) *
                        coeffs_grad[a];
                    GradVJy_TM += (pVpsArray_TM[K][J + a][Ii] -
                                   pVpsArray_TM[K][J - a][Ii]) *
                                  coeffs_grad[a];
                  }
                  ...
                  if (pmvAtmConstraint[index_mvatm] == 1) {
                    GradBJx += (pBArray[K - o][J - o][Ii - o + a] -
                                pBArray[K - o][J - o][Ii - o - a]) *
                               coeffs_grad[a];
                    GradBJx_TM += (pBArray_TM[K - o][J - o][Ii - o + a] -
                                   pBArray_TM[K - o][J - o][Ii - o - a]) *
                                  coeffs_grad[a];
                    GradVJx +=
                        (pVpsArray[K][J][Ii + a] - pVpsArray[K][J][Ii - a]) *
                        coeffs_grad[a];
                    GradVJx_TM += (pVpsArray_TM[K][J][Ii + a] -
                                   pVpsArray_TM[K][J][Ii - a]) *
                                  coeffs_grad[a];
                  }
                }

                PhicmVtmJ = PotPhicArrGlbIdx[k][j][i] - pVpsArray_TM[K][J][Ii];
                PhicpVpsJ = PotPhicArrGlbIdx[k][j][i] + pVpsArray[K][J][Ii];
                BtmpBps = BlcArrGlbIdx_TM[k][j][i] + BlcArrGlbIdx[k][j][i];

                ForceProcComponentz += -GradBJz_TM * PhicmVtmJ -
                                       GradBJz * PhicpVpsJ -
                                       (GradVJz_TM - GradVJz) * BtmpBps -
                                       GradVJz * pVpsArray[K][J][Ii] +
                                       GradVJz_TM * pVpsArray_TM[K][J][Ii];

                ForceProcComponenty += -GradBJy_TM * PhicmVtmJ -
                                       GradBJy * PhicpVpsJ -
                                       (GradVJy_TM - GradVJy) * BtmpBps -
                                       GradVJy * pVpsArray[K][J][Ii] +
                                       GradVJy_TM * pVpsArray_TM[K][J][Ii];

                ForceProcComponentx += -GradBJx_TM * PhicmVtmJ -
                                       GradBJx * PhicpVpsJ -
                                       (GradVJx_TM - GradVJx) * BtmpBps -
                                       GradVJx * pVpsArray[K][J][Ii] +
                                       GradVJx_TM * pVpsArray_TM[K][J][Ii];
              }
  ...
  for (poscnt = 0; poscnt < pSddft->nAtoms; poscnt++) {
    ...
    pSddft->pForces_corr[index_force++] =
        -0.5 * delta * delta * delta * FORCE_corr;
...
PetscErrorCode Force_Nonlocal(SDDFT_OBJ *pSddft, Mat *Psi) {
  ...
  PetscScalar tableR[MAX_TABLE_SIZE], tableUlDeltaV[4][MAX_TABLE_SIZE],
      coeffs_grad[MAX_ORDER + 1], tableU[4][MAX_TABLE_SIZE];
  ...
  PetscInt i = 0, j, k, xi, yj, zk, offset, Ii, J, K, II, JJ, KK;
  PetscInt nzVps, nyVps, nxVps, nzVpsloc, nyVpsloc, nxVpsloc, i0, j0, k0,
      poscnt, index = 0, at, ii, jj, kk, l, m, index_mvatm = 0;
  PetscInt o = pSddft->order;
  ...
  MatProductCreate(pSddft->gradient_x, *Psi, NULL, &pSddft->YOrbNew);
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (lmax != 0) {
      max1 = PetscRealPart(pSddft->psd[at].rc_s) >
                     PetscRealPart(pSddft->psd[at].rc_p)
                 ? pSddft->psd[at].rc_s
                 : pSddft->psd[at].rc_p;
      //
      max2 = PetscRealPart(pSddft->psd[at].rc_d) >
                     PetscRealPart(pSddft->psd[at].rc_f)
                 ? pSddft->psd[at].rc_d
                 : pSddft->psd[at].rc_f;
      ...
      for (l = 0; l <= lmax; l++) {
        /*
         * since the pseudopotential table read from the file does not have a
         * value at r=0, for s orbital, we assume the nonlocal projector and the
         * pseudowavefunction value at r=0 to be same as that calculated from
         * the first entry read from the file. However for p,d and f orbitals,
      ...
      do {
        ...
        count++;
               ...
               PetscRealPart(1.732 * pSddft->CUTOFF[at] + 10.0));
      rmax = tableR[count - 1];
      ...
      for (l = 0; l <= lmax; l++) {
        ...
        /*
         * derivatives of the spline fit to the projectors an
         * pseudowavefunctions
         */
        ...
        if (l != lloc) {
          ...
          while (PetscRealPart(rtemp) <= PetscRealPart(Rcut + delta)) {
            ispline_gen(tableR, tableUlDeltaV[l], count, &rtemp, &UlDelVl,
                        &Dtemp, 1, YDUlDeltaV[l]);
            ispline_gen(tableR, tableU[l], count, &rtemp, &Ul, &Dtemp, 1,
                        YDU[l]);
      ...
      for (poscnt = start; poscnt <= end; poscnt++) {
        ...
        if (pmvAtmConstraint[index_mvatm] == 1 ||
          ...
          /*
           * find if domain of influence of pseudopotential overlaps with the
           * domain stored by processor
           */
          if (xs >= xcor && xs <= xcor + lxdim - 1)
            xstart = xs;
          else if (xcor >= xs && xcor <= xl)
          ...
          if ((xstart != -1) && (xend != -1) && (ystart != -1) &&
              (yend != -1) && (zstart != -1) && (zend != -1))
            overlap = 1;
          ...
          if (overlap) {
            ...
            /*
             * Get local pointer (column wise) to the wavefunctions and gradient
             * of wavefunctions
             */
                  ...
                  for (l = 0; l <= lmax; l++) {
                    if (l != lloc) {
                      if (r == tableR[0]) {
                        ...
                        ispline_gen(tableR, tableUlDeltaV[l], count, &r,
                                    &pUlDeltaVl, &Dtemp, 1, YDUlDeltaV[l]);
                      }
                      if (l == 0)
                        Rcut = pSddft->psd[at].rc_s;
                      if (l == 1)
                      ...
                      for (m = -l; m <= l; m++) {
                        SpHarmonic = SphericalHarmonic(x - x0, y - y0, z - z0,
                                                       l, m, Rcut + 1.0);
                        for (ii = 0; ii < pSddft->Nstates; ii++) {
                          OrbScalingFactor =
                              1.0 / (sqrt(delta * delta * delta) * norms[ii]);
                          if (pmvAtmConstraint[index_mvatm] == 1)
                            W[poscnt][ii][l][m + l] +=
                                OrbScalingFactor *
                                (arrGradPsiSeq[LIrow - rowStart +
                                               (rowEnd - rowStart) * ii]) *
                                pUlDeltaVl * SpHarmonic;
                          T[poscnt][ii][l][m + l] +=
                              OrbScalingFactor *
                              (arrPsiSeq[LIrow - rowStart +
                                         (rowEnd - rowStart) * ii]) *
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (lmax != 0) {
      ...
      for (poscnt = start; poscnt <= end; poscnt++) {
        fJ_x = 0.0;
        for (i = 0; i < pSddft->Nstates; i++) {
          gi = smearing_FermiDirac(pSddft->Beta, pSddft->lambda[i],
                                   pSddft->lambda_f);
          for (l = 0; l <= lmax; l++) {
            if (l != lloc) {
              for (m = -l; m <= l; m++) {
                ierr = MPI_Allreduce(MPI_IN_PLACE, &T[poscnt][i][l][m + l], 1,
                                     MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                CHKERRQ(ierr);
                ierr = MPI_Allreduce(MPI_IN_PLACE, &W[poscnt][i][l][m + l], 1,
                                     MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                CHKERRQ(ierr);
  ...
  /* NOTE: we do no need to calculate T since it has already been calculated
   * while calculating the x-component of forces. Hence we do not need to
   * calculate the exact denominator. Also, memory for W has already been
   * created, we dont need to allocate again, just need to set it to zero.
   */
  ...
  MatProductCreate(pSddft->gradient_y, *Psi, NULL, &pSddft->YOrbNew);
  MatProductSetType(pSddft->YOrbNew, MATPRODUCT_AB);
  MatProductSetFromOptions(pSddft->YOrbNew);
  MatProductSymbolic(pSddft->YOrbNew);
  MatProductNumeric(pSddft->YOrbNew);
  ...
  /* we do no need to calculate T since it has already been calculated while
   * calculating the x-component of forces. Hence we do not need to calculate
   * the exact denominator. Also, memory for W has already been created, we dont
   * need to allocate again, just need to set it to zero
   */
  ...
  MatProductCreate(pSddft->gradient_z, *Psi, NULL, &pSddft->YOrbNew);
  MatProductSetType(pSddft->YOrbNew, MATPRODUCT_AB);
  MatProductSetFromOptions(pSddft->YOrbNew);
  MatProductSymbolic(pSddft->YOrbNew);
  MatProductNumeric(pSddft->YOrbNew);
...
void Display_force(SDDFT_OBJ *pSddft) {
  ...
  PetscScalar Ct =
      27.211384523 / 0.529177249; // Hartree/Bohr to eV/Angstrom conversion
...
///////////////////////////////////////////////////////////////////////////////////////////////
//         Symmetrysize_force: Symmetricizes forces such that sum of forces is
//         zero          //
///////////////////////////////////////////////////////////////////////////////////////////////
...
void PeriodicCalculate_force(SDDFT_OBJ *pSddft) {
  ...
  PetscInt i = 0, j, k, xi, yj, zk, offset, tablesize, l, Ii, J, K;
  PetscInt p, nzVps, nyVps, nxVps, nzBJ, nyBJ, nxBJ, nzGradBJ, nyGradBJ,
      nxGradBJ, i0, j0, k0, a, poscnt, index = 0, index_force = 0,
                                       index_mvatm = 0, count, at;
  PetscScalar delta = pSddft->delta, x, y, z, r, cutoffr, rmax;
  PetscInt n_x = pSddft->numPoints_x;
  PetscInt n_y = pSddft->numPoints_y;
  PetscInt n_z = pSddft->numPoints_z;
      ...
      for (PP = -Nposx; PP <= Nposx; PP++)
        for (QQ = -Nposy; QQ <= Nposy; QQ++)
          for (RR = -Nposz; RR <= Nposz; RR++) {
            ...
            /*
             * find if domain of influence of pseudocharge overlaps with the
             * domain stored by processor
             */
            if (xs >= xcor && xs <= xcor + lxdim - 1)
              xstart = xs;
            ...
            if (overlap) {
              /*
               * find the starting and ending indices of finite difference nodes
               * in each direction for calculation of pseudopotential and
                ...
                for (j = 0; j < nyVps; j++)
                  for (i = 0; i < nxVps; i++) {
                    ...
                    if (r == tableR[0]) {
                      ...
                      ispline_gen(tableR, tableVps, count, &r,
                                  &pVpsArray[k][j][i], &Dtemp, 1, YD);
                    }
                  }

              /*
               * calculate pseudocharge density at nodes inside the overlap
               * region from previously calculated pseudopotential values using
...
///////////////////////////////////////////////////////////////////////////////////////////////
//     PeriodicCalculate_forceCorrection: force correction on atoms for periodic
//     systems     //
///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for (QQ = -Nposy; QQ <= Nposy; QQ++)
          for (RR = -Nposz; RR <= Nposz; RR++) {
            ...
            if (overlap) {
              ...
              /*
               * calculate pseudocharge density and contribution to self energy
               * at nodes inside the overlap region from previously calculated
               * pseudopotential values using finite difference stencil
               */
              for (k = ZBJstart; k <= ZBJend; k++)
                for (j = YBJstart; j <= YBJend; j++)
                  for (i = XBJstart; i <= XBJend; i++) {
                    ...
                    pBArray[k - ZBJstart][j - YBJstart][i - XBJstart] = Bval;
                    pBArray_TM[k - ZBJstart][j - YBJstart][i - XBJstart] =
                        Bval_TM;
                  }
              ...
              for (k = Zstart; k <= Zend; k++)
                for (j = Ystart; j <= Yend; j++)
                  for (i = Xstart; i <= Xend; i++) {
                    ...
                    PhicmVtmJ =
                        PotPhicArrGlbIdx[k][j][i] - pVpsArray_TM[K][J][Ii];
                    PhicpVpsJ = PotPhicArrGlbIdx[k][j][i] + pVpsArray[K][J][Ii];
                    BtmpBps = BlcArrGlbIdx_TM[k][j][i] + BlcArrGlbIdx[k][j][i];
...
///////////////////////////////////////////////////////////////////////////////////////////////
//  PeriodicForce_Nonlocal: nonlocal component of force on atoms for periodic
//  systems        //
///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for (PP = -Nposx; PP <= Nposx; PP++)
          for (QQ = -Nposy; QQ <= Nposy; QQ++)
            for (RR = -Nposz; RR <= Nposz; RR++) {
              ...
              /*
               * find if domain of influence of pseudopotential overlaps with
               * the domain stored by processor
               */
              if (xs >= xcor && xs <= xcor + lxdim - 1)
                xstart = xs;
              else if (xcor >= xs && xcor <= xl)
              ...
              if (overlap) {
                ...
                /*
                 * Get local pointer (column wise) to the wavefunctions and
                 * gradient of wavefunctions
                 */
                MatDenseGetArray(*Psi, &arrPsiSeq);
                MatDenseGetArray(pSddft->YOrbNew, &arrGradPsiSeq);

                /*
                 * evaluate the the quantities that contribute to nonlocal force
                 * at nodes in the overlap region
                 */
                for (k = zstart; k <= zend; k++)
                  for (j = ystart; j <= yend; j++)
                    for (i = xstart; i <= xend; i++) {
                        ...
                        if (l != lloc) {
                          ...
                          for (m = -l; m <= l; m++) {
                            SpHarmonic = SphericalHarmonic(
                                x - x0, y - y0, z - z0, l, m, Rcut + 1.0);
                            for (ii = 0; ii < pSddft->Nstates; ii++) {
                              OrbScalingFactor =
                                  1.0 /
                                  (sqrt(delta * delta * delta) * norms[ii]);
                              W[poscnt][ii][l][m + l] +=
                                  OrbScalingFactor *
                                  (arrGradPsiSeq[LIrow - rowStart +
                                                 (rowEnd - rowStart) * ii]) *
    ...
    if (lmax != 0) {
      ...
      do {
        ...
        for (l = 0; l <= lmax; l++) {
          /*
           * since the pseudopotential table read from the file does not have a
           * value at r=0, for s orbital, we assume the nonlocal projector and
           * the pseudowavefunction value at r=0 to be same as that calculated
           * from the first entry read from the file. However for p,d and f
           * orbitals, we assume the values at r=0 to be 0.
           */
          if (l == 0) {
            tableUlDeltaV[0][count] = pSddft->psd[at].Us[count - 1] *
                                      (pSddft->psd[at].Vs[count - 1] -
          ...
          for (QQ = -Nposy; QQ <= Nposy; QQ++)
            for (RR = -Nposz; RR <= Nposz; RR++) {
              ...
              if (overlap) {
                ...
                /*
                 * Get local pointer (column wise) to the gradient of
                 * wavefunctions
                 */
                MatDenseGetArray(pSddft->YOrbNew, &arrGradPsiSeq);
...
///////////////////////////////////////////////////////////////////////////////////////////////
//   kPointPeriodicForce_Nonlocal: nonlocal component of force on atoms for
//   k-point code     //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode kPointPeriodicForce_Nonlocal(SDDFT_OBJ *pSddft) {
  ...
  int OrbitalSize, rank;
  PetscInt Nposx, Nposy, Nposz, PP, QQ,
      RR; // number of positions we need to move the atom to get the right
          // forces.
  ...
  for (k = 0; k < pSddft->Nkpts_sym; k++) {
    ...
    MatProductCreate(pSddft->gradient_x, pSddft->XOrb1[k], NULL, &pSddft->YOrb1[k]);
    ...
    MatProductCreate(pSddft->gradient_x, pSddft->XOrb2[k], NULL, &pSddft->YOrb2[k]);
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (lmax != 0) {
      ...
      for (poscnt = start; poscnt <= end; poscnt++) {
        ...
        for (k = 0; k < pSddft->Nkpts_sym; k++) {
          PetscMalloc(sizeof(PetscScalar **) * (pSddft->Nstates),
                      &W1[poscnt][k]);
          PetscMalloc(sizeof(PetscScalar **) * (pSddft->Nstates),
                      &T1[poscnt][k]);
          PetscMalloc(sizeof(PetscScalar **) * (pSddft->Nstates),
          ...
          PetscMalloc(sizeof(PetscScalar **) * (pSddft->Nstates),
                      &T2[poscnt][k]);
          if (W1[poscnt][k] == NULL || T1[poscnt][k] == NULL ||
              W2[poscnt][k] == NULL || T2[poscnt][k] == NULL) {
            printf("W[][] || T");
            exit(1);
          }
          for (i = 0; i < pSddft->Nstates; i++) {
            ...
            if (W1[poscnt][k][i] == NULL || T1[poscnt][k][i] == NULL ||
                W2[poscnt][k][i] == NULL || T2[poscnt][k][i] == NULL) {
              printf("W[][][] || T");
              exit(1);
            }

            for (l = 0; l <= lmax; l++) {
              PetscMalloc(sizeof(PetscScalar) * (2 * l + 1),
                          &W1[poscnt][k][i][l]);
              PetscMalloc(sizeof(PetscScalar) * (2 * l + 1),
                          &T1[poscnt][k][i][l]);
              PetscMalloc(sizeof(PetscScalar) * (2 * l + 1),
              ...
              PetscMalloc(sizeof(PetscScalar) * (2 * l + 1),
                          &T2[poscnt][k][i][l]);
              if (W1[poscnt][k][i][l] == NULL || T1[poscnt][k][i][l] == NULL ||
                  W2[poscnt][k][i][l] == NULL || T2[poscnt][k][i][l] == NULL) {
                printf("W[][][][] T[][][][]");
                exit(1);
              }
              for (m = -l; m <= l; m++) {
                ...
                for (k = zstart; k <= zend; k++)
                  for (j = ystart; j <= yend; j++)
                    for (i = xstart; i <= xend; i++) {
                      ...
                      /*
                       * coordinates of the "original finite difference node
                       * (unshifted, not periodic image)"
                       */
                      X = x - PP * 2.0 * R_x;
                      Y = y - QQ * 2.0 * R_y;
                      Z = z - RR * 2.0 * R_z;
                      ...
                      for (l = 0; l <= lmax; l++) {
                        if (l != lloc) {
                          ...
                          for (m = -l; m <= l; m++) {
                            ComplexSphericalHarmonic(x - x0, y - y0, z - z0, l,
                                                     m, Rcut + 1.0, &SH_real,
                                                     &SH_imag);
                            ...
                            for (nk1 = 1; nk1 <= pSddft->Kx; nk1++)
                              for (nk2 = 1; nk2 <= pSddft->Ky; nk2++)
                                for (nk3 = 1; nk3 <= ceil(pSddft->Kz / 2.0);
                                     nk3++) {
                                  k1 = ((2.0 * nk1 - pSddft->Kx - 1.0) /
                                        (2.0 * pSddft->Kx)) *
                                       (M_PI / R_x);
                                  k2 = ((2.0 * nk2 - pSddft->Ky - 1.0) /
                                        (2.0 * pSddft->Ky)) *
                                       (M_PI / R_y);
                                  k3 = ((2.0 * nk3 - pSddft->Kz - 1.0) /
                                        (2.0 * pSddft->Kz)) *
                                       (M_PI / R_z);

                                  MatDenseGetArray(pSddft->XOrb1[kpt],
                                                   &arrPsi1Seq);
                                  MatDenseGetArray(pSddft->XOrb2[kpt],
                                                   &arrPsi2Seq);
                                  MatDenseGetArray(pSddft->YOrb1[kpt],
                                                   &arrGradPsi1Seq);
                                  MatDenseGetArray(pSddft->YOrb2[kpt],
                                                   &arrGradPsi2Seq);

                                  for (ii = 0; ii < pSddft->Nstates; ii++) {
                                    OrbScalingFactor =
                                        1.0 / (sqrt(delta * delta * delta) *
                                               sqrt(norms[ii] * norms[ii] +
                                                    norms1[ii] * norms1[ii]));
                                    a1 = OrbScalingFactor *
                                         (cos(k1 * X + k2 * Y + k3 * Z) *
                                              arrGradPsi1Seq
                                    ...
                                    b1 = OrbScalingFactor *
                                         (cos(k1 * X + k2 * Y + k3 * Z) *
                                              arrPsi1Seq[LIrow - rowStart +
                                                         (rowEnd - rowStart) *
                                                             ii] -
                                          sin(k1 * X + k2 * Y + k3 * Z) *
                                              arrPsi2Seq[LIrow - rowStart +
                                                         (rowEnd - rowStart) *
                                                             ii]);
                                    b2 = OrbScalingFactor *
                                         (sin(k1 * X + k2 * Y + k3 * Z) *
                                    ...
                                    W1[poscnt][kpt][ii][l][m + l] +=
                                        (a1 * pUlDeltaVl * SH_real +
                                         a2 * pUlDeltaVl * SH_imag);
                                    W2[poscnt][kpt][ii][l][m + l] +=
                                        (a2 * pUlDeltaVl * SH_real -
                                         a1 * pUlDeltaVl * SH_imag);

                                    T1[poscnt][kpt][ii][l][m + l] +=
                                        (b1 * pUlDeltaVl * SH_real +
                                         b2 * pUlDeltaVl * SH_imag) /
                                        Dexact[l];
                                    T2[poscnt][kpt][ii][l][m + l] +=
                                        (b1 * pUlDeltaVl * SH_imag -
                                         b2 * pUlDeltaVl * SH_real) /
                                        Dexact[l];
                                  }
                                  MatDenseRestoreArray(pSddft->XOrb1[kpt],
                                                       &arrPsi1Seq);
                                  MatDenseRestoreArray(pSddft->XOrb2[kpt],
                                                       &arrPsi2Seq);
                                  MatDenseRestoreArray(pSddft->YOrb1[kpt],
                                                       &arrGradPsi1Seq);
                                  MatDenseRestoreArray(pSddft->YOrb2[kpt],
                                                       &arrGradPsi2Seq);
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (lmax != 0) {
      ...
      for (poscnt = start; poscnt <= end; poscnt++) {
        ...
        for (k = 0; k < pSddft->Nkpts_sym; k++) {
          for (i = 0; i < pSddft->Nstates; i++) {
            gi = smearing_FermiDirac(pSddft->Beta, pSddft->lambdakpt[k][i],
                                     pSddft->lambda_f); // occupation
            for (l = 0; l <= lmax; l++) {
              if (l != lloc) {
                for (m = -l; m <= l; m++) {
                  ierr =
                      MPI_Allreduce(MPI_IN_PLACE, &T1[poscnt][k][i][l][m + l],
                                    1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                  CHKERRQ(ierr);
                  ierr =
                      MPI_Allreduce(MPI_IN_PLACE, &W1[poscnt][k][i][l][m + l],
                                    1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                  CHKERRQ(ierr);
                  ierr =
                      MPI_Allreduce(MPI_IN_PLACE, &T2[poscnt][k][i][l][m + l],
                                    1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                  CHKERRQ(ierr);
                  ierr =
                      MPI_Allreduce(MPI_IN_PLACE, &W2[poscnt][k][i][l][m + l],
                                    1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                  CHKERRQ(ierr);
                  fJ_x +=
                      gi * pSddft->kptWts[k] *
                      (W1[poscnt][k][i][l][m + l] * T1[poscnt][k][i][l][m + l] -
                       W2[poscnt][k][i][l][m + l] * T2[poscnt][k][i][l][m + l]);
                }
              }
            }
          }
  ...
  for (k = 0; k < pSddft->Nkpts_sym; k++) {
    ...
    MatProductCreate(pSddft->gradient_y, pSddft->XOrb1[k], NULL, &pSddft->YOrb1[k]);
    MatProductSetType(pSddft->YOrb1[k], MATPRODUCT_AB);
    MatProductSetFromOptions(pSddft->YOrb1[k]);
    MatProductSymbolic(pSddft->YOrb1[k]);
    MatProductNumeric(pSddft->YOrb1[k]);
    ...
    MatProductCreate(pSddft->gradient_y, pSddft->XOrb2[k], NULL, &pSddft->YOrb2[k]);
    MatProductSetType(pSddft->YOrb2[k], MATPRODUCT_AB);
    MatProductSetFromOptions(pSddft->YOrb2[k]);
    MatProductSymbolic(pSddft->YOrb2[k]);
    MatProductNumeric(pSddft->YOrb2[k]);
  ...
  for (k = 0; k < pSddft->Nkpts_sym; k++) {
    ...
    MatProductCreate(pSddft->gradient_z, pSddft->XOrb1[k], NULL, &pSddft->YOrb1[k]);
    MatProductSetType(pSddft->YOrb1[k], MATPRODUCT_AB);
    MatProductSetFromOptions(pSddft->YOrb1[k]);
    MatProductSymbolic(pSddft->YOrb1[k]);
    MatProductNumeric(pSddft->YOrb1[k]);
    ...
    MatProductCreate(pSddft->gradient_z, pSddft->XOrb1[k], NULL, &pSddft->YOrb2[k]);
    MatProductSetType(pSddft->YOrb2[k], MATPRODUCT_AB);
    MatProductSetFromOptions(pSddft->YOrb2[k]);
    MatProductSymbolic(pSddft->YOrb2[k]);
    MatProductNumeric(pSddft->YOrb2[k]);
