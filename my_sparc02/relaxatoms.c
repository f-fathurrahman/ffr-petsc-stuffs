...
PetscErrorCode FormFunction_relaxAtoms(SDDFT_OBJ *pSddft) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, " ***  Start of DFT calculation for fixed "
                                "atomic positions (NONPERIODIC)***\n");
  ...
  pSddft->RelaxCount++;
  PetscPrintf(PETSC_COMM_WORLD, "***   End of DFT calculation for fixed atomic "
                                "positions  (NON-PERIODIC)***\n");

  return 0;
}

//   GammaPoint_FormFunction_relaxAtoms: performs one electronic structure
//   minimization for  //
//                         fixed atomic positions (gamma point formulation) //

PetscErrorCode GammaPoint_FormFunction_relaxAtoms(SDDFT_OBJ *pSddft) {
  PetscPrintf(PETSC_COMM_WORLD, " ***  Start of DFT calculation for fixed "
                                "atomic positions (GAMMA POINT PERIODIC)***\n");
  ...
  pSddft->RelaxCount++;
  PetscPrintf(PETSC_COMM_WORLD, "***   End of DFT calculation for fixed atomic "
                                "positions  (GAMMA POINT PERIODIC)***\n");

  return 0;
}

//   kPoint_FormFunction_relaxAtoms: performs one electronic structure
//   minimization for  //
...
PetscErrorCode kPoint_FormFunction_relaxAtoms(SDDFT_OBJ *pSddft) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, " ***  Start of DFT calculation for fixed "
                                "atomic positions (k-point PERIODIC)***\n");
  ...
  pSddft->RelaxCount++;
  PetscPrintf(PETSC_COMM_WORLD, "***   End of DFT calculation for fixed atomic "
                                "positions  (k POINT PERIODIC)***\n");

  return 0;
}

//    Solve_ElectronicStructureProblem: call respective  electronic structure
//    minimization   //
...
//    NLCG_relaxAtoms: Nonlinear Conjugate Gradient method for ground state
//    minimization     //
//             Reference: An Introduction to the Conjugate Gradient Method
//             Without           //
...
PetscErrorCode SDDFT_Periodic(SDDFT_OBJ *pSddft) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, "</div> EXIT SDDFT_Periodic\n");

  return 0;
}

//     SDDFT_kPointPeriodic: function for nonperiodic Density Functional theory
//     calculation  //
...
//     Periodic_MapAtoms: Mapping outside atoms back into the domain using
//     periodic mapping  //
...
void Periodic_MapAtoms(SDDFT_OBJ *pSddft) {
  ...
  /*
   * we assume that the an atom can only move to a periodic cell which is an
   * immediate neighbor of the simulation domain
   */
  if (pSddft->BC == 2) {
    VecGetArray(pSddft->Atompos, &pAtompos);
