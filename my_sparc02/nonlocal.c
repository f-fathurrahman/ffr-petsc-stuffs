...
PeriodicLaplacianNonlocalPseudopotential_MatInit(SDDFT_OBJ *pSddft) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 619 in nonlocal.c\n");

  PetscMalloc(sizeof(MatStencil) * (o * 6 + 1), &Lapcol);
  PetscMalloc(sizeof(PetscScalar) * (o * 6 + 1), &Lapval);

  PetscPrintf(PETSC_COMM_WORLD, "Pass here 624 in nonlocal.c\n");
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 672 in nonlocal.c\n");
   ...
   */
  VecGetArray(pSddft->Atompos, &pAtompos);
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 682 in nonlocal.c\n");
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (lmax != 0) {
      ...
      //
      PetscPrintf(PETSC_COMM_WORLD, "Pass here 766\n");
      //
      /*
       * Computation of denominator term
       */
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 1058 in nonlocal.c\n");
...
// the nonlocal pseudopotential sparse matrix
