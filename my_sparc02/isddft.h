...
typedef struct {
  ...
  PetscInt *noa, *noe; ///< For each type of atom, noa stores the number of atoms of that type, noe stores the number of
                       ///< valence electrons of that type
  ...
  PetscReal range_x;       ///< stores half of domain length in x direction
  PetscReal range_y;       ///< stores half of domain length in y direction
  PetscReal range_z;       ///< stores half of domain length in z direction
  ...
  PetscReal PseudochargeRadiusTOL; ///< stores the tolerance for calculation of pseudocharge density radius
  ...
  PetscInt SCFNewRhoCalcCtr; ///< stores the number of times Chebychev filtering is done in the first SCF iteration
                             ///< before updation of electron density
  ...
  PetscReal *CUTOFF;    ///< stores the pseudocharge cutoff \f$ r_J^b \f$. Size of array is nAtoms.
  PetscInt *startPos;   ///< stores the starting position of a particular type of atom in the list of atom positions
                        ///< (stored in Vec Atompos)
  PetscInt *endPos; ///< stores the ending position of a particular type of atom in the list of atom positions (stored
                    ///< in Vec Atompos)
  char **atomType;  ///< stores the string containing the atom type name for every type of atom. example H for Hydrogen,
                    ///< He for Helium, Li for Lithium, etc. This is read from the .ion file
  char **psdName;   ///< stores the string containing the pseudopotential name for every type of atom. This is read from
                    ///< the .inpt file
  int *localPsd; ///< stores the respective local component of pseudopotential for each type of atom. 0 for s, 1 for p,
                 ///< 2 for d, 3 for f.
  ...
  Vec chrgDensB;     ///< PETSc vector for storing the pseudocharge density. This is calculated from the pseudopotential
                     ///< files given as user input
  Vec chrgDensB_TM;  ///< PETSc vector for storing the reference pseudocharge density. This is used for calculation of
                     ///< energy and force corrections
  Vec potentialPhi;  ///< PETSc vector for storing the electrostatic potential
  Vec twopiRhoPB;    ///< PETSc vector for storing \f$ 2*\pi*(\rho+b) \f$ (the right hand side of the poisson equation
                     ///< solves for the elctrostatic potential)
  ...
  Vec twopiBTMmBPS;  ///< PETSc vector for storing the difference between the reference pseudocharge density and the
  ...
  Vec forces; ///< PETSc vector storing the forces on the atoms. Size of this vector is 3*nAtoms. The first, second and
              ///< third entries are the x,y and z coordinates of the atomic forces on first atom, the fourth, fifth and
              ///< sixth entries are the x,y,z coordinates on the atomic positions of second atom and so on.
  Vec Atompos; ///< PETSc vector storing the atomic positions. Size of this vector is 3*nAtoms. The first, second and
               ///< third entries are the x,y and z coordinates of the atomic positions of first atom, the fourth, fifth
               ///< and sixth entries are the x,y,z coordinates of the atomic positions of second atom and so on.
  Vec mvAtmConstraint; ///< PETSc vector storing the constraints for movement of atoms. Size of this vector is 3*nAtoms.
                       ///< An entry of 1 means the atom is "movable", hence "net" force on it is non-zero, an entry of
                       ///< 0 means the atom is "fixed" and the "net" force on it is zero.

  Mat laplaceOpr; ///< PETSc distributed sparse matrix (row major format) for storing -1/2 laplacian. This is a PETSc
                  ///< DMDA type matrix. See PETSc manuals for details on DMDA matrices
  Mat HamiltonianOpr; ///< PETSc distributed sparse matrix (row major format) for storing the Hamiltonian operator
  Mat gradient_x;     ///< PETSc distributed sparse matrix (row major format) for storing x component of the gradient
                      ///< operator. This is a PETSc DMDA type matrix. See PETSc manuals for details on DMDA matrices
  Mat gradient_y;     ///< PETSc distributed sparse matrix (row major format) for storing y component of the gradient
                      ///< operator. This is a PETSc DMDA type matrix. See PETSc manuals for details on DMDA matrices
  Mat gradient_z;     ///< PETSc distributed sparse matrix (row major format) for storing z component of the gradient
                      ///< operator. This is a PETSc DMDA type matrix. See PETSc manuals for details on DMDA matrices

  /*
   * matrices for storing orbitals
   */
  Mat XOrb;    ///< PETSc distributed dense matrix (row major format) for storing the orbitals. Stores input to the
               ///< chebychev filter
  Mat YOrb;    ///< PETSc distributed dense matrix (row major format) for storing the orbitals. Stores the filtered
               ///< orbitals
  Mat YOrbNew; ///< PETSc distributed dense matrix (row major format) for storing the orbitals. Stores a temporary copy
               ///< of the orbitals in chebychev filtering
  ...
  Mat HamiltonianOpr1; ///< PETSc distributed sparse matrix (row major format) for storing the real part of k-point
                       ///< Hamiltonian
  Mat HamiltonianOpr2; ///< PETSc distributed sparse matrix (row major format) for storing the imaginary part of k-point
                       ///< Hamiltonian
  ...
  KSP ksp; ///< PETSc (Krylov subspace) ksp object for solving the Poisson equation. See PETSc manual for more details
           ///< on ksp
