...
void Read_parameters(SDDFT_OBJ *pSddft) {
  ...
  do {
    ...
    if (strcmp(str, "FD_ORDER:") == 0) {
      ...
      if (pSddft->Ntype <= 0) {
        printf("Input number of types of elements first\n");
        exit(1);
      }
      for (i = 0; i < pSddft->Ntype; i++) {
...
void Read_ion(SDDFT_OBJ *pSddft) {
  ...
  do {
    ...
    PetscPrintf(PETSC_COMM_WORLD, "at=%d  Ntype=%d\n", at, pSddft->Ntype);

    if (fscanf(fConfFile, "%10s", pSddft->atomType[at]) == EOF) // atom type (string)
      break;
...
/*
  while( !feof(fConfFile) && (at < pSddft->Ntype)) {
    ...
    // loop over number of atoms of a particular type and read their positions
    PetscPrintf(PETSC_COMM_WORLD, "at=%d noa[at]=%d\n", at, pSddft->noa[at]);
    for (poscnt = 0; poscnt < pSddft->noa[at]; poscnt++) {
      pSddft->noetot += pSddft->noe[at];
      fscanf(fConfFile, "%lf", &x0);
      fscanf(fConfFile, "%lf", &y0);
...
*/
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    if (fscanf(fConfFile, "%s", pSddft->atomType[at]) != 1)
        break;

    if (fscanf(fConfFile, "%d", &pSddft->noe[at]) != 1)
        break;

    if (fscanf(fConfFile, "%d", &pSddft->noa[at]) != 1)
        break;
  ...
  fclose(fConfFile);
  if (at != pSddft->Ntype) {
    // cout << "Number of blocks: " << at << " in .ion file not same as number of atom type:" << pSddft->Ntype << "in
    // .inpt file\n";
    exit(1);
  }
...
void Read_relax(SDDFT_OBJ *pSddft) {
  ...
  if (pSddft->RelaxFlag == 1) {
    ...
    if (at != pSddft->Ntype) {
      // cout<< "Number of blocks: "<< at <<" in .relax file not same as number of atom type: "<< pSddft->Ntype << " in
      // .inpt file\n";
      exit(1);
    }
  }
