...
void SddftObjInitialize(SDDFT_OBJ *pSddft) {
  ...
  //  For petsc-3.12
  PetscOptionsGetString(PETSC_NULLPTR, PETSC_NULLPTR, "-name", pSddft->file, sizeof(pSddft->file), PETSC_NULLPTR);
}
...
PetscErrorCode Objects_Create(SDDFT_OBJ *pSddft) {
  ...
  if (pSddft->BC == 1) {
     ...
     */
    PetscPrintf(PETSC_COMM_WORLD, "Using PBC\n");
    if (comm_size == 1) {
      DMDACreate3d(PETSC_COMM_SELF, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC, DMDA_STENCIL_STAR,
                   n_x, n_y, n_z, PETSC_DECIDE, PETSC_DECIDE, PETSC_DECIDE, 1, o, 0, 0, 0, &pSddft->da);
      ...
      DMDACreate3d(PETSC_COMM_WORLD, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC,
                   DMDA_STENCIL_STAR, n_x, n_y, n_z, PETSC_DECIDE, PETSC_DECIDE, PETSC_DECIDE, 1, o, 0, 0, 0,
                   &pSddft->da);
  ...
  PetscCall( DMSetUp(pSddft->da) );
  PetscCall( DMSetUp(pSddft->da_grad) );
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 83\n");

  PetscCall( DMCreateGlobalVector(pSddft->da, &pSddft->elecDensRho) );
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 86\n");

  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->SuperposAtRho) );
  PetscPrintf(PETSC_COMM_WORLD, "Pass here 90\n");

  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->chrgDensB) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->chrgDensB_TM) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->potentialPhi) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->Phi_c) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->twopiRhoPB) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->twopiBTMmBPS) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->Veff) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->bjVj) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->bjVj_TM) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->Vxc) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->tempVec) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->PoissonRHSAdd) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->xkprev) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->xk) );
  PetscCall( VecDuplicate(pSddft->elecDensRho, &pSddft->fkprev) );
...
void ChargDensB_cutoff(SDDFT_OBJ *pSddft) {
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    /*
     * now go over different radii by incrementing radius by 1 bohr, unless
     * relative error in charge is below tolerence
     */
    do {
...
PetscErrorCode Wavefunctions_MatInit(SDDFT_OBJ *pSddft) {
  ...
  PetscErrorCode ierr;
  // PetscInt Npts = pSddft->numPoints_x * pSddft->numPoints_y *
  // pSddft->numPoints_z;
...
// kPointWavefunctions_MatInit: Initialize matrices for storing k point
// wavefunctions  //
...
//  Wavefunctions_MatMatMultSymbolic: Symbolic matrix matrix multiplication for
//  the orbitals //
...
PetscScalar kpointWeight(PetscScalar kx, PetscScalar ky, PetscScalar kz) {
   ...
   * we find the weight of the z-direction k point. If the kz has 0, then the
