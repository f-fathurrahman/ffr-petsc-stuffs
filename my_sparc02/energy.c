...
void kPointSystemEnergy_Calc(SDDFT_OBJ *pSddft) {
  ...
  for (k = 0; k < pSddft->Nkpts_sym; k++)
    for (i = 0; i < pSddft->Nstates; i++) {
      Eband = Eband + (2.0 / pSddft->Nkpts) * pSddft->kptWts[k] *
                          smearing_FermiDirac(pSddft->Beta, pSddft->lambdakpt[k][i], pSddft->lambda_f) *
                          pSddft->lambdakpt[k][i];
    }
  pSddft->Eband = Eband;
...
void ConstShift_Calc(SDDFT_OBJ *pSddft) {
  ...
  PetscScalar tableR[MAX_TABLE_SIZE], tableVps[MAX_TABLE_SIZE], r, rmax, rcut = pSddft->REFERENCE_CUTOFF,
                                                                         delta = pSddft->delta;
