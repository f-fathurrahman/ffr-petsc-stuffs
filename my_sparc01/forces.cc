...
PetscErrorCode Force_Nonlocal(SDDFT_OBJ *pSddft, Mat *Psi) {
  ...
  /* Specify multiplication type */
  MatProductSetType(pSddft->YOrbNew, MATPRODUCT_AB);
  /* Let PETSc choose best algorithm */
  MatProductSetFromOptions(pSddft->YOrbNew);
  /* Symbolic phase (allocate sparsity structure) */
  ...
  /* NOTE: we do not need to calculate T since it has already been calculated while
