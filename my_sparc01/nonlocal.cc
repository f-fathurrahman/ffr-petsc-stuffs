...
PetscErrorCode LaplacianNonlocalPseudopotential_MatInit(SDDFT_OBJ *pSddft) {
  ...
  PetscInt xstart = -1, ystart = -1, zstart = -1;
  PetscInt xend = -1, yend = -1, zend = -1, overlap = 0, colidx;
  ...
  PetscScalar dr = 1e-3;
  int OrbitalSize;
  PetscInt i = 0, j, k, xi, yj, zk, offset, I, J, K, II, JJ, KK, nzVps, nyVps, nxVps, nzVpsloc, nyVpsloc, nxVpsloc, i0,
                      ...
                      for (l = 0; l <= lmax; l++) {
                        if (l != lloc) {
                          ...
                          for (m = -l; m <= l; m++) {
                            ...
                            val[colidx] += (pUlDeltaVl1 * SphericalHarmonic(x - x0, y - y0, z - z0, l, m, Rcut) *
    ...
    if (r != 0.0) {
      if (l == 0)
        ...
        if (m == -4)
          ...
          SH = (3.0 / 16.0) * sqrt(1.0 / pi) * (35.0 * z * z * z * z - 30.0 * z * z * r * r + 3.0 * r * r * r * r) /
               (r * r * r * r);
        else if (m == 1)
          ...
          SH = (3.0 / 16.0) * sqrt(35.0 / pi) * (x * x * (x * x - 3.0 * y * y) - y * y * (3.0 * x * x - y * y)) /
               (r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
          exit(1);
        ...
        if (m == -5)
          SH = (3.0 * sqrt(2.0) / 32.0) *
               (8.0 * x * x * x * x * y - 4.0 * x * x * y * y * y + 4.0 * pow(y, 5) - 3.0 * y * p * p * p * p) /
               (r * r * r * r * r);
        else if (m == -4)
          SH = (3.0 / 16.0) * sqrt(385.0 / pi) * (4.0 * x * x * x * y - 4.0 * x * y * y * y) * z / (r * r * r * r * r);
        else if (m == -3)
          SH = (sqrt(2.0 * 385.0 / pi) / 32.0) * (3.0 * y * p * p - 4.0 * y * y * y) * (9 * z * z - r * r) /
               (r * r * r * r * r);
        else if (m == -2)
          SH = (1.0 / 8.0) * sqrt(1155.0 / pi) * 2.0 * x * y * (3.0 * z * z * z - z * r * r) / (r * r * r * r * r);
        else if (m == -1)
          SH = (1.0 / 16.0) * sqrt(165.0 / pi) * y * (21.0 * z * z * z * z - 14.0 * r * r * z * z + r * r * r * r) /
               (r * r * r * r * r);
        else if (m == 0)
          SH = (1.0 / 16.0) * sqrt(11.0 / pi) *
               (63.0 * z * z * z * z * z - 70.0 * z * z * z * r * r + 15.0 * z * r * r * r * r) / (r * r * r * r * r);
        else if (m == 1)
          SH = (1.0 / 16.0) * sqrt(165.0 / pi) * x * (21.0 * z * z * z * z - 14.0 * r * r * z * z + r * r * r * r) /
               (r * r * r * r * r);
        else if (m == 2)
          SH = (1.0 / 8.0) * sqrt(1155.0 / pi) * (x * x - y * y) * (3.0 * z * z * z - r * r * z) / (r * r * r * r * r);
        else if (m == 3)
          SH = (sqrt(2.0 * 385.0 / pi) / 32.0) * (4.0 * x * x * x - 3.0 * p * p * x) * (9.0 * z * z - r * r) /
               (r * r * r * r * r);
        else if (m == 4)
          SH = (3.0 / 16.0) * sqrt(385.0 / pi) * (4.0 * (x * x * x * x - y * y * y * y) - 3.0 * p * p * p * p) * z /
               (r * r * r * r * r);
        else if (m == 5)
          SH = (3.0 * sqrt(2.0) / 32.0) * sqrt(77.0 / pi) *
               (4.0 * x * x * x * x * x + 8.0 * x * y * y * y * y - 4.0 * x * x * x * y * y - 3.0 * x * p * p * p * p) /
               (r * r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
          exit(1);
        ...
        if (m == -6)
          SH = (sqrt(2.0 * 3003.0 / pi) / 64.0) *
               (12.0 * pow(x, 5) * y + 12.0 * x * pow(y, 5) - 8.0 * x * x * x * y * y * y - 6.0 * x * y * pow(p, 4)) /
               (r * r * r * r * r * r);
        else if (m == -5)
          SH = (3.0 / 32.0) * sqrt(2.0 * 1001.0 / pi) *
               (8.0 * pow(x, 4) * y - 4.0 * x * x * y * y * y + 4.0 * pow(y, 5) - 3.0 * y * pow(p, 4)) * z /
               (r * r * r * r * r * r);
        else if (m == -4)
          SH = (3.0 / 32.0) * sqrt(91.0 / pi) * (4.0 * x * x * x * y - 4.0 * x * y * y * y) * (11.0 * z * z - r * r) /
               (r * r * r * r * r * r);
        else if (m == -3)
          SH = (sqrt(2.0 * 1365.0 / pi) / 32.0) * (-4.0 * y * y * y + 3.0 * y * p * p) *
               (11.0 * z * z * z - 3.0 * z * r * r) / (r * r * r * r * r * r);
        else if (m == -2)
          SH = (sqrt(2.0 * 1365 / pi) / 64.0) * (2.0 * x * y) * (33.0 * pow(z, 4) - 18.0 * z * z * r * r + pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == -1)
          SH = (sqrt(273.0 / pi) / 16.0) * y * (33.0 * pow(z, 5) - 30.0 * z * z * z * r * r + 5.0 * z * pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == 0)
          SH = (sqrt(13.0 / pi) / 32.0) *
               (231.0 * pow(z, 6) - 315 * pow(z, 4) * r * r + 105.0 * z * z * pow(r, 4) - 5.0 * pow(r, 6)) /
               (r * r * r * r * r * r);
        else if (m == 1)
          SH = (sqrt(273.0 / pi) / 16.0) * x * (33.0 * pow(z, 5) - 30.0 * z * z * z * r * r + 5 * z * pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == 2)
          SH = (sqrt(2.0 * 1365 / pi) / 64.0) * (x * x - y * y) *
               (33.0 * pow(z, 4) - 18.0 * z * z * r * r + pow(r, 4)) / (r * r * r * r * r * r);
        else if (m == 3)
          SH = (sqrt(2.0 * 1365.0 / pi) / 32.0) * (4.0 * x * x * x - 3.0 * x * p * p) *
               (11.0 * z * z * z - 3.0 * z * r * r) / (r * r * r * r * r * r);
        else if (m == 4)
          SH = (3.0 / 32.0) * sqrt(91.0 / pi) * (4.0 * pow(x, 4) + 4.0 * pow(y, 4) - 3.0 * pow(p, 4)) *
               (11.0 * z * z - r * r) / (r * r * r * r * r * r);
        else if (m == 5)
          SH = (3.0 / 32.0) * sqrt(2.0 * 1001.0 / pi) *
               (4.0 * pow(x, 5) + 8.0 * x * pow(y, 4) - 4.0 * x * x * x * y * y - 3.0 * x * pow(p, 4)) * z /
               (r * r * r * r * r * r);
        else if (m == 6)
          SH = (sqrt(2.0 * 3003.0 / pi) / 64.0) *
               (4.0 * pow(x, 6) - 4.0 * pow(y, 6) + 12.0 * x * x * pow(y, 4) - 12.0 * pow(x, 4) * y * y +
                3.0 * y * y * pow(p, 4) - 3.0 * x * x * pow(p, 4)) /
               (r * r * r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
          exit(1);
...
///////////////////////////////////////////////////////////////////////////////////////////////
PetscScalar SphericalHarmonic_Derivatives(PetscScalar x, PetscScalar y, PetscScalar z, int l, int m, PetscScalar rc,
        ...
        for (k = zstart; k <= zend; k++)
          for (j = ystart; j <= yend; j++)
            for (i = xstart; i <= xend; i++) {
              ...
              pOffDiagArray[K][J][I] =
                  pOffDiagArray[K][J][I] + (nxVps * nyVps * nzVps - nxVpsloc * nyVpsloc * nzVpsloc);
            }
      }
    }
  }
