...
PetscErrorCode MultipoleExpansion_Phi(SDDFT_OBJ *pSddft, Vec *RhopBvec) {
  ...
  for (l = 0; l <= lmax; l++) {
    for (m = 0; m < (l + 1); m++) {
      ierr = MPI_Allreduce(MPI_IN_PLACE, &Q1[l][m], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      CHKERRQ(ierr);
      Q1[l][m] = delta * delta * delta * Q1[l][m];
      Q2[l][0] = Q1[l][0];
      if (m != 0) {
        ierr = MPI_Allreduce(MPI_IN_PLACE, &Q2[l][m], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        CHKERRQ(ierr);
  ...
  if (flag == 1) {
    printf("WARNING: In Multipole expansion, charge at boundary has not decayed. Might need to rerun calculation with "
           "a larger domain size if you see this warning in the last SCF iteration.\n");
  }
...
PetscErrorCode CalculateDipoleMoment(SDDFT_OBJ *pSddft) {
  ...
  ierr = MPI_Allreduce(MPI_IN_PLACE, &DMx, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ierr = MPI_Allreduce(MPI_IN_PLACE, &DMy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ierr = MPI_Allreduce(MPI_IN_PLACE, &DMz, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ...
  PetscPrintf(PETSC_COMM_WORLD,
              "Dipole moment (due to only electron density) in Debye: DMx=%0.16lf, DMy=%0.16lf, DMz=%0.16lf \n",
