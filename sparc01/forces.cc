...
PetscErrorCode Force_Nonlocal(SDDFT_OBJ* pSddft,Mat* Psi)
{
  ...
  PetscInt i=0,j,k,xi,yj,zk,offset,I,J,K,II,JJ,KK,nzVps,nyVps,nxVps;
  PetscInt nzVpsloc, nyVpsloc,nxVpsloc,i0,j0,k0, poscnt, index=0, at, ii,jj,kk, l,m, index_mvatm=0;
  PetscInt o = pSddft->order;
...
void  Symmetrysize_force(SDDFT_OBJ* pSddft)
{
  ...
  for(poscnt=0;poscnt<pSddft->nAtoms;poscnt++)
    {
      SumFx+=pForces[Index]; SumFy+=pForces[Index+1]; SumFz+=pForces[Index+2];
      Index=Index+3;
    }
  Index=0;
  /*
