/*=============================================================================================
  ...
  the correction | term using multipole expansion
  |
...
///////////////////////////////////////////////////////////////////////////////////////////////
//     MultipoleExpansion_Phi: multipole expansion to find the correct boundary
//     condition    //
...
PetscErrorCode MultipoleExpansion_Phi(SDDFT_OBJ *pSddft, Vec *RhopBvec) {
        ...
        for (l = 0; l <= lmax; l++)
          for (m = 0; m < (l + 1); m++) {
            ...
            if (m != 0) {
              ...
              Q2[l][m] +=
                  (pRhopBArray[k][j][i] * pow(r, 1.0 * l) * SpHarmoniclmm);
            }
          }
      }
  ...
  for (l = 0; l <= lmax; l++) {
    for (m = 0; m < (l + 1); m++) {
      ierr = MPI_Allreduce(MPI_IN_PLACE, &Q1[l][m], 1, MPI_DOUBLE, MPI_SUM,
                           MPI_COMM_WORLD);
      CHKERRQ(ierr);
      Q1[l][m] = delta * delta * delta * Q1[l][m];
      Q2[l][0] = Q1[l][0];
      if (m != 0) {
        ierr = MPI_Allreduce(MPI_IN_PLACE, &Q2[l][m], 1, MPI_DOUBLE, MPI_SUM,
                             MPI_COMM_WORLD);
        CHKERRQ(ierr);
  ...
  /*
   * define regions of the domain which contributes to the charge correction.
   * i.e. 0 to 0+o and nx-o nx in each directions.
   */
  XS[0] = 0;
  XL[0] = 0 + o;
  YS[0] = 0;
  ...
  for (poscnt = 0; poscnt < 6; poscnt++) {
    ...
    if (overlap) {
      /*
       * if poscnt==0, then we need to calculate phi using multipole expansion
       * at points inside [xstart-o,-1,ystart,yend,zstart,zend] if poscnt==1,
       * then we need to calculate phi using multipole expansion at points
       * inside [n_x,xend+o,ystart,yend,zstart,zend]
       * if poscnt==2, then we need to calculate phi using multipole expansion
       * at points inside [xstart,xend,ystart-o,-1,zstart,zend] if poscnt==3,
       * then we need to calculate phi using multipole expansion at points
       * inside [xstart,xend,n_y,yend+o,zstart,zend]
       * if poscnt==4, then we need to calculate phi using multipole expansion
       * at points inside [xstart,xend,ystart,yend,zstart-o,-1] if poscnt==5,
       * then we need to calculate phi using multipole expansion at points
      ...
      /*
       * calculate the electrostatic potential using multipole expansion at
       * points inside the overlap region
       */
      for (k = 0; k < nZPhi; k++)
        for (j = 0; j < nYPhi; j++)
          for (i = 0; i < nXPhi; i++) {
            ...
            for (l = 0; l <= lmax; l++) {
              ...
              pPhiArray[k][j][i] += ((4.0 * M_PI) / (2.0 * l + 1.0)) *
                                    (1.0 / pow(r, 1.0 * l + 1.0)) * Phi_l;
            }
          }
            ...
            for (a = 1; a <= o; a++) {
              if (poscnt == 0) {
                if ((i - a) < 0) // this means node is outside left boundary
                {
                  pRHSVec[k][j][i] +=
                      -coeffs[a] *
                      pPhiArray[k - Zstart][j - Ystart][i - Xstart - a];
                }
              } else if (poscnt == 1) {
                ...
                if ((j - a) < 0) // this means node is outside bottom boundary
                {
                  pRHSVec[k][j][i] +=
                      -coeffs[a] *
                      pPhiArray[k - Zstart][j - Ystart - a][i - Xstart];
                }
              } else if (poscnt == 3) {
                ...
                if ((k - a) < 0) // this means node is outside front boundary
                {
                  pRHSVec[k][j][i] +=
                      -coeffs[a] *
                      pPhiArray[k - Zstart - a][j - Ystart][i - Xstart];
                }
              } else if (poscnt == 5) {
  ...
  if (flag == 1) {
    printf("WARNING: In Multipole expansion, charge at boundary has not "
           "decayed. Might need to rerun calculation with a larger domain size "
           "if you see this warning in the last SCF iteration.\n");
  }
...
PetscErrorCode CalculateDipoleMoment(SDDFT_OBJ *pSddft) {
  ...
  ierr =
      MPI_Allreduce(MPI_IN_PLACE, &DMx, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ierr =
      MPI_Allreduce(MPI_IN_PLACE, &DMy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ierr =
      MPI_Allreduce(MPI_IN_PLACE, &DMz, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ...
  PetscPrintf(PETSC_COMM_WORLD,
              "Dipole moment (due to only electron density) in Debye: "
              "DMx=%0.16lf, DMy=%0.16lf, DMz=%0.16lf \n",
