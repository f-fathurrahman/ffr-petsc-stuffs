/*=============================================================================================
  ...
  | file name: scf.cc
  |
  | Description: This file contains the functions required for self consistent
  field iteration | and subspace projection
  |
...
PetscScalar SelfConsistentField(SDDFT_OBJ *pSddft) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, "**********************************************"
                                "*****************************\n");
  PetscPrintf(PETSC_COMM_WORLD, "                          Self Consistent "
                                "Field                            \n");
  PetscPrintf(PETSC_COMM_WORLD, "**********************************************"
                                "*****************************\n");
  ...
  VecWAXPY(pSddft->Veff, 1.0, pSddft->Vxc,
           pSddft->potentialPhi); // effective potential

  VecCopy(pSddft->Veff, pSddft->xk); // this is required for mixing

  while ( PetscRealPart(error) > PetscRealPart(pSddft->TOLSCF) &&
         count <= (pSddft->MAXITSCF - pSddft->SCFNewRhoCalcCtr)) {

    /*
     * add effective potential to Hamiltonian. Need to be subtracted out at the
     * end of SCF
     */
    MatDiagonalSet(pSddft->HamiltonianOpr, pSddft->Veff, ADD_VALUES);

    /*
     * find smallest and largest eigenvalues of Hamiltonian using lanczos
     * algorithm
     */
    Lanczos(pSddft, &EigenMin, &EigenMax);
    ChebyshevFiltering(pSddft, pSddft->ChebDegree, lambda_cutoff, EigenMax,
                       EigenMin);
    ...
    LAPACKE_dsygv(LAPACK_COL_MAJOR, 1, 'V', 'U', Nstates, arrHsub, Nstates,
                  arrMsub, Nstates, pSddft->lambda);
    ...
    /*
     * subtract the effective potential from the Hamiltonian that was previously
     * added
     */
  ...
  PetscPrintf(PETSC_COMM_WORLD,
              "Convergence of Self Consistent Field iterations achieved! \n");
  PetscPrintf(PETSC_COMM_WORLD, "**********************************************"
                                "*****************************\n\n");
  PetscPrintf(PETSC_COMM_WORLD, "**********************************************"
                                "*****************************\n");
...
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode ProjectMatrices(SDDFT_OBJ *pSddft, Mat *Psi, Mat *Hsub,
                               Mat *Msub) {
   ...
   */
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha,
              arrPsiSeq, K, arrPsiSeq, K, beta, arrSubMat, M);

  /*
   * sum entries from all processors
   */
  ierr = MPI_Allreduce(MPI_IN_PLACE, arrSubMat, Nstates * Nstates, MPI_DOUBLE,
   ...
   */
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha,
              arrPsiSeq, K, arrHPsiSeq, K, beta, arrSubMat, M);
...
///////////////////////////////////////////////////////////////////////////////////////////////
//       kPointSelfConsistentField: Self Consistent Field iteration for k point
//       sampling     //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscScalar kPointSelfConsistentField(SDDFT_OBJ *pSddft) {
  ...
  Mat Msub1, Hsub1;
  Mat Msub2, Hsub2;
  PetscScalar *arrHsub, *arrMsub; // for storing the values in an array (column
                                  // wise) and passing to lapack functions
  int i;
    ...
    for (nk1 = 1; nk1 <= pSddft->Kx; nk1++)
      for (nk2 = 1; nk2 <= pSddft->Ky; nk2++)
        for (nk3 = 1; nk3 <= ceil(pSddft->Kz / 2.0); nk3++) {
           ...
           */
          k1 = ((2.0 * nk1 - pSddft->Kx - 1.0) / (2.0 * pSddft->Kx)) *
               (M_PI / R_x);
          k2 = ((2.0 * nk2 - pSddft->Ky - 1.0) / (2.0 * pSddft->Ky)) *
               (M_PI / R_y);
          k3 = ((2.0 * nk3 - pSddft->Kz - 1.0) / (2.0 * pSddft->Kz)) *
               (M_PI / R_z);
           ...
           */
          if (count == 0) {
            MatMatMultSymbolic(pSddft->HamiltonianOpr1, pSddft->XOrb1[k],
                               PETSC_DEFAULT, &pSddft->YOrb1[k]);
            MatMatMultNumeric(pSddft->HamiltonianOpr1, pSddft->XOrb1[k],
                              pSddft->YOrb1[k]);

            MatMatMultSymbolic(pSddft->HamiltonianOpr2, pSddft->XOrb2[k],
                               PETSC_DEFAULT, &pSddft->ZOrb1);
            MatMatMultNumeric(pSddft->HamiltonianOpr2, pSddft->XOrb2[k],
                              pSddft->ZOrb1);

            MatMatMultSymbolic(pSddft->HamiltonianOpr1, pSddft->XOrb2[k],
                               PETSC_DEFAULT, &pSddft->YOrb2[k]);
            MatMatMultNumeric(pSddft->HamiltonianOpr1, pSddft->XOrb2[k],
                              pSddft->YOrb2[k]);

            MatMatMultSymbolic(pSddft->HamiltonianOpr2, pSddft->XOrb1[k],
                               PETSC_DEFAULT, &pSddft->ZOrb2);
            MatMatMultNumeric(pSddft->HamiltonianOpr2, pSddft->XOrb1[k],
                              pSddft->ZOrb2);

            MatMatMultSymbolic(pSddft->HamiltonianOpr1, pSddft->YOrb1[k],
                               PETSC_DEFAULT, &pSddft->YOrbNew1);
            MatMatMultNumeric(pSddft->HamiltonianOpr1, pSddft->YOrb1[k],
                              pSddft->YOrbNew1);

            MatMatMultSymbolic(pSddft->HamiltonianOpr2, pSddft->YOrb2[k],
                               PETSC_DEFAULT, &pSddft->ZOrbNew1);
            MatMatMultNumeric(pSddft->HamiltonianOpr2, pSddft->YOrb2[k],
                              pSddft->ZOrbNew1);

            MatMatMultSymbolic(pSddft->HamiltonianOpr1, pSddft->YOrb2[k],
                               PETSC_DEFAULT, &pSddft->YOrbNew2);
            MatMatMultNumeric(pSddft->HamiltonianOpr1, pSddft->YOrb2[k],
                              pSddft->YOrbNew2);

            MatMatMultSymbolic(pSddft->HamiltonianOpr2, pSddft->YOrb1[k],
                               PETSC_DEFAULT, &pSddft->ZOrbNew2);
            MatMatMultNumeric(pSddft->HamiltonianOpr2, pSddft->YOrb1[k],
                              pSddft->ZOrbNew2);
          }
           ...
           */
          kPointChebyshevFiltering(pSddft, pSddft->ChebDegree, lambda_cutoff,
                                   EigenMax, EigenMin, k);

          /*
           * calculate projected Hamiltonian and Overlap matrix
           */
          kPointProjectMatrices(pSddft, &pSddft->YOrb1[k], &pSddft->YOrb2[k],
                                &Hsub1, &Hsub2, &Msub1, &Msub2);

          /*
           * solve the generalized eigenvalue problem for eigenvalues and
           * eigenvectors
           */
           ...
           * subspace rotation
           */
          kPointRotatePsi(pSddft, &pSddft->YOrb1[k], &pSddft->YOrb2[k], &Hsub1,
                          &Hsub2, &pSddft->XOrb1[k], &pSddft->XOrb2[k]);
...
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode kPointProjectMatrices(SDDFT_OBJ *pSddft, Mat *U1, Mat *U2,
                                     Mat *Hsub1, Mat *Hsub2, Mat *Msub1,
                                     Mat *Msub2) {
  /*
   * Psi is the matrix used for projecting
   * Hsub is the subspace Hamiltonian
   ...
   * multiplying sequential parts of matrices
   */
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha, arrUSeq1,
              K, arrUSeq1, K, beta, arrSubMat1, M);
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha, arrUSeq2,
              K, arrUSeq2, K, beta, arrSubMat2, M);

  /*
   * sum entries from all processors
   */
  ierr = MPI_Allreduce(MPI_IN_PLACE, arrSubMat1, Nstates * Nstates, MPI_DOUBLE,
                       MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
  ierr = MPI_Allreduce(MPI_IN_PLACE, arrSubMat2, Nstates * Nstates, MPI_DOUBLE,
                       MPI_SUM, MPI_COMM_WORLD);
  CHKERRQ(ierr);
   ...
   */
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha, arrUSeq1,
              K, arrHUSeq1, K, beta, arrSubMat1, M);
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, M, N, K, alpha, arrUSeq2,
              K, arrHUSeq2, K, beta, arrSubMat2, M);
...
///////////////////////////////////////////////////////////////////////////////////////////////
//    SolveGeneralizedEigen: solve subspace eigenvalue problem for k point
//    sampling method   //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode SolveGeneralizedEigen(SDDFT_OBJ *pSddft, Mat *Hsub1, Mat *Hsub2,
                                     Mat *Msub1, Mat *Msub2, int k) {
  PetscErrorCode ierr;
  PetscScalar *arrReal;
  PetscScalar *arrImag;
  PetscInt Nstates = pSddft->Nstates;
  ...
  PetscMalloc(sizeof(lapack_complex_double) *
                  (pSddft->Nstates * pSddft->Nstates),
              &arrHsub);
  PetscMalloc(sizeof(lapack_complex_double) *
                  (pSddft->Nstates * pSddft->Nstates),
              &arrMsub);
  ...
  for (i = 0; i < Nstates * Nstates; i++) {
    __real__ arrMsub[i] = (double)arrReal[i];
    __imag__ arrMsub[i] = (double)arrImag[i];
  }
  ...
  for (i = 0; i < Nstates * Nstates; i++) {
    __real__ arrHsub[i] = (double)arrReal[i];
    __imag__ arrHsub[i] = (double)arrImag[i];
  }
  info = LAPACKE_zhegv(LAPACK_COL_MAJOR, 1, 'V', 'U', Nstates, arrHsub, Nstates,
                       arrMsub, Nstates, w);
  if (info != 0) {
    printf("Error in eigenvalue problem! rank=%d, value returned from "
           "LAPACKE_zhegv =%d \n",
           rank, info);
  }
   ...
   */
  for (i = 0; i < Nstates * Nstates; i++) {
    arrReal[i] = __real__ arrHsub[i];
    arrImag[i] = __imag__ arrHsub[i];
  }
