/*=============================================================================================
  ...
  nonlocal | pseudopotential part of the Hamiltonian matrix and spherical
  harmonics required for the | calculation of nonlocal projectors
  |
...
///////////////////////////////////////////////////////////////////////////////////////////////
//  LaplacianNonlocalPseudopotential_MatInit: creates -(1/2)laplacian+Vnonloc
//  operator       //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode LaplacianNonlocalPseudopotential_MatInit(SDDFT_OBJ *pSddft) {
  ...
  /*
   * create the distributed matrix with the same communication pattern as the
   * laplacian operator
   */
    ...
    for (j = ycor; j < ycor + lydim; j++)
      for (i = xcor; i < xcor + lxdim; i++) {
        ...
        MatSetValuesStencil(pSddft->HamiltonianOpr, 1, &Laprow, 6 * o + 1,
                            Lapcol, Lapval, INSERT_VALUES);
      }
  ierr = MatAssemblyBegin(pSddft->HamiltonianOpr, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(pSddft->HamiltonianOpr, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    /*
     * if the atom is hydrogen, then we omit the calculation of nonlocal
     * pseudopotential
     */
    if (lmax == 0) {
    ...
    if (lmax != 0) {
      ...
      for (poscnt = start; poscnt <= end; poscnt++) {
        ...
        if (overlap) {
          ...
          k0 = zs;
          /*
           * while evaluating the nonlocal pseudopotential entries, each
           * processor enters matrix entries in local rows. local rows
           * commensurate with the overlapping nodes. For every local row, we go
           * over all the nodes in the pseudopotential cutoff region and
            ...
            for (j = ystart; j <= yend; j++)
              for (i = xstart; i <= xend; i++) {
                ...
                for (kk = zs; kk <= zl; kk++)
                  for (jj = ys; jj <= yl; jj++)
                    for (ii = xs; ii <= xl; ii++) {
                      ...
                      yy = delta * jj - R_y;
                      zz = delta * kk - R_z;
                      r2 = sqrt((xx - x0) * (xx - x0) + (yy - y0) * (yy - y0) +
                                (zz - z0) * (zz - z0));
                      val[colidx] = 0;
                      for (l = 0; l <= lmax; l++) {
                        if (l != lloc) {
                          if (r1 == tableR[0]) {
                            ...
                            ispline_gen(tableR, tableUlDeltaV[l], count, &r1,
                                        &pUlDeltaVl1, &DUlDelVl, 1,
                                        YDUlDeltaV[l]);
                          }
                          ...
                          if (r2 == tableR[0]) {
                            ...
                            ispline_gen(tableR, tableUlDeltaV[l], count, &r2,
                                        &pUlDeltaVl2, &DUlDelVl, 1,
                                        YDUlDeltaV[l]);
                          }
                          for (m = -l; m <= l; m++) {
                            SpHarmonic = SphericalHarmonic(x - x0, y - y0,
                                                           z - z0, l, m, Rcut);
                            val[colidx] +=
                                (pUlDeltaVl1 *
                                 SphericalHarmonic(x - x0, y - y0, z - z0, l, m,
                                                   Rcut) *
                                 pUlDeltaVl2 *
                                 SphericalHarmonic(xx - x0, yy - y0, zz - z0, l,
                                                   m, Rcut)) /
                                Dexact[l];
                          }
                        }
                ...
                if (colidx > 0) {
                  ...
                  ierr = MatSetValues(pSddft->HamiltonianOpr, 1, &LIrow, colidx,
                                      LIcol, val, ADD_VALUES);
                  CHKERRQ(ierr);
                }
              }
          PetscFree(val);
  ...
  ierr = MatAssemblyBegin(pSddft->HamiltonianOpr, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(pSddft->HamiltonianOpr, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
...
///////////////////////////////////////////////////////////////////////////////////////////////
// PeriodicLaplacianNonlocalPseudopotential_MatInit: creates
// -(1/2)laplacian+Vnonloc operator//
//                  for periodic boundary conditions (without k point sampling)
//                  //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode
PeriodicLaplacianNonlocalPseudopotential_MatInit(SDDFT_OBJ *pSddft) {
  PetscScalar *pAtompos;
  PetscInt xcor, ycor, zcor, gxdim, gydim, gzdim, lxdim, lydim, lzdim;
  PetscInt xs, ys, zs, xl, yl, zl, xstart = -1, ystart = -1, zstart = -1,
                                   xend = -1, yend = -1, zend = -1, overlap = 0,
          ...
          for (QQ = Imin_y; QQ <= Imax_y; QQ++)
            for (RR = Imin_z; RR <= Imax_z; RR++) {
              ...
              if (overlap) {
                ...
                PetscMalloc(sizeof(PetscScalar) * (nzVps * nyVps * nxVps),
                            &val);
                PetscMalloc(sizeof(PetscInt) * (nzVps * nyVps * nxVps), &LIcol);
                 ...
                 * while evaluating the nonlocal pseudopotential entries, each
                 * processor enters matrix entries in local rows. local rows
                 * commensurate with the overlapping nodes. For every local row,
                 * we go over all the nodes in the pseudopotential cutoff region
                 * and calculate the matrix entries
                 */
                      ...
                      for (kk = zs; kk <= zl; kk++)
                        for (jj = ys; jj <= yl; jj++)
                          for (ii = xs; ii <= xl; ii++) {
                            ...
                            /*
                             * map points outside the computational domain to
                             * points inside computational domain
                             */
...
///////////////////////////////////////////////////////////////////////////////////////////////
//          SphericalHarmonic: returns the real spherical harmonic for some
//          given l,m        //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscScalar SphericalHarmonic(PetscScalar x, PetscScalar y, PetscScalar z,
                              int l, int m, PetscScalar rc) {
  /*
  ...
  PetscScalar SH = 0.0;
  if (PetscRealPart(r) <= PetscRealPart(rc)) {
    if (l == 0)
      SH = C00;
    ...
    if (r != 0.0) {
      if (l == 0)
        ...
        if (m == -4)
          SH = (3.0 / 4.0) * sqrt(35.0 / pi) * (x * y * (x * x - y * y)) /
               (r * r * r * r);
        else if (m == -3)
          SH = (3.0 / 4.0) * sqrt(35.0 / (2.0 * pi)) * (3.0 * x * x - y * y) *
               y * z / (r * r * r * r);
        else if (m == -2)
          SH = (3.0 / 4.0) * sqrt(5.0 / pi) * x * y * (7.0 * z * z - r * r) /
               (r * r * r * r);
        else if (m == -1)
          SH = (3.0 / 4.0) * sqrt(5.0 / (2.0 * pi)) * y * z *
               (7.0 * z * z - 3.0 * r * r) / (r * r * r * r);
        else if (m == 0)
          SH = (3.0 / 16.0) * sqrt(1.0 / pi) *
               (35.0 * z * z * z * z - 30.0 * z * z * r * r +
                3.0 * r * r * r * r) /
               (r * r * r * r);
        else if (m == 1)
          SH = (3.0 / 4.0) * sqrt(5.0 / (2.0 * pi)) * x * z *
               (7.0 * z * z - 3.0 * r * r) / (r * r * r * r);
        else if (m == 2)
          SH = (3.0 / 8.0) * sqrt(5.0 / (pi)) * (x * x - y * y) *
               (7.0 * z * z - r * r) / (r * r * r * r);
        else if (m == 3)
          SH = (3.0 / 4.0) * sqrt(35.0 / (2.0 * pi)) * (x * x - 3.0 * y * y) *
               x * z / (r * r * r * r);
        else if (m == 4)
          SH = (3.0 / 16.0) * sqrt(35.0 / pi) *
               (x * x * (x * x - 3.0 * y * y) - y * y * (3.0 * x * x - y * y)) /
               (r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
          exit(1);
        ...
        p = sqrt(x * x + y * y);
        if (m == -5)
          SH = (3.0 * sqrt(2.0) / 32.0) *
               (8.0 * x * x * x * x * y - 4.0 * x * x * y * y * y +
                4.0 * pow(y, 5) - 3.0 * y * p * p * p * p) /
               (r * r * r * r * r);
        else if (m == -4)
          SH = (3.0 / 16.0) * sqrt(385.0 / pi) *
               (4.0 * x * x * x * y - 4.0 * x * y * y * y) * z /
               (r * r * r * r * r);
        else if (m == -3)
          SH = (sqrt(2.0 * 385.0 / pi) / 32.0) *
               (3.0 * y * p * p - 4.0 * y * y * y) * (9 * z * z - r * r) /
               (r * r * r * r * r);
        else if (m == -2)
          SH = (1.0 / 8.0) * sqrt(1155.0 / pi) * 2.0 * x * y *
               (3.0 * z * z * z - z * r * r) / (r * r * r * r * r);
        else if (m == -1)
          SH = (1.0 / 16.0) * sqrt(165.0 / pi) * y *
               (21.0 * z * z * z * z - 14.0 * r * r * z * z + r * r * r * r) /
               (r * r * r * r * r);
        else if (m == 0)
          SH = (1.0 / 16.0) * sqrt(11.0 / pi) *
               (63.0 * z * z * z * z * z - 70.0 * z * z * z * r * r +
                15.0 * z * r * r * r * r) /
               (r * r * r * r * r);
        else if (m == 1)
          SH = (1.0 / 16.0) * sqrt(165.0 / pi) * x *
               (21.0 * z * z * z * z - 14.0 * r * r * z * z + r * r * r * r) /
               (r * r * r * r * r);
        else if (m == 2)
          SH = (1.0 / 8.0) * sqrt(1155.0 / pi) * (x * x - y * y) *
               (3.0 * z * z * z - r * r * z) / (r * r * r * r * r);
        else if (m == 3)
          SH = (sqrt(2.0 * 385.0 / pi) / 32.0) *
               (4.0 * x * x * x - 3.0 * p * p * x) * (9.0 * z * z - r * r) /
               (r * r * r * r * r);
        else if (m == 4)
          SH = (3.0 / 16.0) * sqrt(385.0 / pi) *
               (4.0 * (x * x * x * x - y * y * y * y) - 3.0 * p * p * p * p) *
               z / (r * r * r * r * r);
        else if (m == 5)
          SH = (3.0 * sqrt(2.0) / 32.0) * sqrt(77.0 / pi) *
               (4.0 * x * x * x * x * x + 8.0 * x * y * y * y * y -
                4.0 * x * x * x * y * y - 3.0 * x * p * p * p * p) /
               (r * r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
          exit(1);
        ...
        p = sqrt(x * x + y * y);
        if (m == -6)
          SH = (sqrt(2.0 * 3003.0 / pi) / 64.0) *
               (12.0 * pow(x, 5) * y + 12.0 * x * pow(y, 5) -
                8.0 * x * x * x * y * y * y - 6.0 * x * y * pow(p, 4)) /
               (r * r * r * r * r * r);
        else if (m == -5)
          SH = (3.0 / 32.0) * sqrt(2.0 * 1001.0 / pi) *
               (8.0 * pow(x, 4) * y - 4.0 * x * x * y * y * y +
                4.0 * pow(y, 5) - 3.0 * y * pow(p, 4)) *
               z / (r * r * r * r * r * r);
        else if (m == -4)
          SH = (3.0 / 32.0) * sqrt(91.0 / pi) *
               (4.0 * x * x * x * y - 4.0 * x * y * y * y) *
               (11.0 * z * z - r * r) / (r * r * r * r * r * r);
        else if (m == -3)
          SH = (sqrt(2.0 * 1365.0 / pi) / 32.0) *
               (-4.0 * y * y * y + 3.0 * y * p * p) *
               (11.0 * z * z * z - 3.0 * z * r * r) / (r * r * r * r * r * r);
        else if (m == -2)
          SH = (sqrt(2.0 * 1365 / pi) / 64.0) * (2.0 * x * y) *
               (33.0 * pow(z, 4) - 18.0 * z * z * r * r + pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == -1)
          SH = (sqrt(273.0 / pi) / 16.0) * y *
               (33.0 * pow(z, 5) - 30.0 * z * z * z * r * r +
                5.0 * z * pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == 0)
          SH = (sqrt(13.0 / pi) / 32.0) *
               (231.0 * pow(z, 6) - 315 * pow(z, 4) * r * r +
                105.0 * z * z * pow(r, 4) - 5.0 * pow(r, 6)) /
               (r * r * r * r * r * r);
        else if (m == 1)
          SH = (sqrt(273.0 / pi) / 16.0) * x *
               (33.0 * pow(z, 5) - 30.0 * z * z * z * r * r +
                5 * z * pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == 2)
          SH = (sqrt(2.0 * 1365 / pi) / 64.0) * (x * x - y * y) *
               (33.0 * pow(z, 4) - 18.0 * z * z * r * r + pow(r, 4)) /
               (r * r * r * r * r * r);
        else if (m == 3)
          SH = (sqrt(2.0 * 1365.0 / pi) / 32.0) *
          ...
          SH = (3.0 / 32.0) * sqrt(91.0 / pi) *
               (4.0 * pow(x, 4) + 4.0 * pow(y, 4) - 3.0 * pow(p, 4)) *
               (11.0 * z * z - r * r) / (r * r * r * r * r * r);
        else if (m == 5)
          SH = (3.0 / 32.0) * sqrt(2.0 * 1001.0 / pi) *
               (4.0 * pow(x, 5) + 8.0 * x * pow(y, 4) -
                4.0 * x * x * x * y * y - 3.0 * x * pow(p, 4)) *
               z / (r * r * r * r * r * r);
        else if (m == 6)
          SH = (sqrt(2.0 * 3003.0 / pi) / 64.0) *
               (4.0 * pow(x, 6) - 4.0 * pow(y, 6) + 12.0 * x * x * pow(y, 4) -
                12.0 * pow(x, 4) * y * y + 3.0 * y * y * pow(p, 4) -
                3.0 * x * x * pow(p, 4)) /
               (r * r * r * r * r * r);
        else {
          printf("incorrect l: %d,m: %d\n", l, m);
...
///////////////////////////////////////////////////////////////////////////////////////////////
//  SphericalHarmonic_Derivatives: returns the real spherical harmonic and their
//  derivatives //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscScalar SphericalHarmonic_Derivatives(PetscScalar x, PetscScalar y,
                                          PetscScalar z, int l, int m,
                                          PetscScalar rc, PetscScalar *Dx,
                                          PetscScalar *Dy, PetscScalar *Dz) {
    ...
    if (r != 0.0) {
      if (l == 0) {
        ...
        if (m == -3) {
          ...
          der1 = (3 * c1 * x * y * (2 * r * r - 3 * x * x + y * y)) /
                 (4.0 * r * r * r * r * r);
          der2 =
              (3 * c1 *
               (-3 * x * x * y * y + y * y * y * y + r * r * (x * x - y * y))) /
              (4.0 * r * r * r * r * r);
          der3 = (-3 * c1 * (3 * x * x * y - y * y * y) * z) /
                 (4.0 * r * r * r * r * r);
        } else if (m == -2) {
          ...
          der1 =
              (c3 * x * y * (r * r - 15 * z * z)) / (4.0 * r * r * r * r * r);
          der2 = -(c3 * (r * r * r * r + 15 * y * y * z * z -
                         r * r * (y * y + 5 * z * z))) /
                 (4.0 * r * r * r * r * r);
          der3 = (c3 * y * z * (11 * r * r - 15 * z * z)) /
                 (4.0 * r * r * r * r * r);
        } else if (m == 0) {
          val = (c4 * (-3 * r * r * z + 5 * z * z * z)) / (4.0 * r * r * r);
          der1 = (3 * c4 * x * z * (r * r - 5 * z * z)) /
                 (4.0 * r * r * r * r * r);
          der2 = (3 * c4 * y * z * (r * r - 5 * z * z)) /
                 (4.0 * r * r * r * r * r);
          der3 = (-3 * c4 *
                  (r * r * r * r - 6 * r * r * z * z + 5 * z * z * z * z)) /
                 (4.0 * r * r * r * r * r);
        } else if (m == 1) {
          val = -(c3 * x * (r * r - 5 * z * z)) / (4.0 * r * r * r);
          der1 = -(c3 * (r * r * r * r + 15 * x * x * z * z -
                         r * r * (x * x + 5 * z * z))) /
                 (4.0 * r * r * r * r * r);
          der2 =
              (c3 * x * y * (r * r - 15 * z * z)) / (4.0 * r * r * r * r * r);
          der3 = (c3 * x * z * (11 * r * r - 15 * z * z)) /
                 (4.0 * r * r * r * r * r);
        } else if (m == 2) {
          val = (c2 * (x * x - y * y) * z) / (4.0 * r * r * r);
          der1 = (c2 * x * (2 * r * r - 3 * x * x + 3 * y * y) * z) /
                 (4.0 * r * r * r * r * r);
          der2 = -(c2 * y * (2 * r * r + 3 * x * x - 3 * y * y) * z) /
                 (4.0 * r * r * r * r * r);
          der3 = (c2 * (x * x - y * y) * (r * r - 3 * z * z)) /
                 (4.0 * r * r * r * r * r);
        } else if (m == 3) {
          val = (c1 * (x * x * x - 3 * x * y * y)) / (4.0 * r * r * r);
          der1 =
              (3 * c1 *
               (-x * x * x * x + 3 * x * x * y * y + r * r * (x * x - y * y))) /
              (4.0 * r * r * r * r * r);
          der2 = (-3 * c1 * x * y * (2 * r * r + x * x - 3 * y * y)) /
                 (4.0 * r * r * r * r * r);
          der3 = (-3 * c1 * (x * x * x - 3 * x * y * y) * z) /
                 (4.0 * r * r * r * r * r);
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////
//                ComplexSphericalHarmonic: returns the complex spherical
//                harmonics          //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscScalar ComplexSphericalHarmonic(PetscScalar x, PetscScalar y,
                                     PetscScalar z, int l, int m,
                                     PetscScalar rc, PetscScalar *SHreal,
                                     PetscScalar *SHimag) {
  if (m < 0) {
    ...
    *SHreal =
        (pow(-1.0, m) / sqrt(2.0)) * SphericalHarmonic(x, y, z, l, abs(m), rc);
    *SHimag =
        (pow(-1.0, m) / sqrt(2.0)) * SphericalHarmonic(x, y, z, l, -abs(m), rc);
  } else {
...
///////////////////////////////////////////////////////////////////////////////////////////////
//    EstimateNonZerosNonlocalPseudopot: Estimating the number of nonzeros for
//    the           //
...
void EstimateNonZerosNonlocalPseudopot(SDDFT_OBJ *pSddft) {
  ...
  PetscInt i = 0, j, k, xi, yj, zk, offset, Ii, J, K, nzVps, nyVps, nxVps,
           nzVpsloc, nyVpsloc, nxVpsloc, poscnt, at;
  ...
  for (at = 0; at < pSddft->Ntype; at++) {
    ...
    max2 = PetscRealPart(pSddft->psd[at].rc_d) > PetscRealPart(pSddft->psd[at].rc_f) ?
           pSddft->psd[at].rc_d : pSddft->psd[at].rc_f;
    max = PetscRealPart(max1) > PetscRealPart(max2) ? max1 : max2;
    cutoffr = max;
    offset = ceil(cutoffr / delta);
        ...
        for (k = zstart; k <= zend; k++)
          for (j = ystart; j <= yend; j++)
            for (i = xstart; i <= xend; i++) {
              ...
              pDiagArray[K][J][Ii] =
                  pDiagArray[K][J][Ii] + nxVpsloc * nyVpsloc * nzVpsloc;
              pOffDiagArray[K][J][Ii] =
                  pOffDiagArray[K][J][Ii] +
                  (nxVps * nyVps * nzVps - nxVpsloc * nyVpsloc * nzVpsloc);
            }
      }
    }
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////
//  PeriodicEstimateNonZerosNonlocalPseudopot: Estimating the number of nonzeros
//  for the     //
//                         nonlocal pseudopotential sparse matrix //
///////////////////////////////////////////////////////////////////////////////////////////////
void PeriodicEstimateNonZerosNonlocalPseudopot(SDDFT_OBJ *pSddft) {
  ...
  PetscInt i = 0, j, k, xi, yj, zk, offset, Ii, J, K;
  PetscInt nzVps, nyVps, nxVps, nzVpsloc, nyVpsloc, nxVpsloc, poscnt, at;
  PetscScalar delta = pSddft->delta, x, y, z, r, rmax;
  PetscInt n_x = pSddft->numPoints_x;
  PetscInt n_y = pSddft->numPoints_y;
  PetscInt n_z = pSddft->numPoints_z;
              ...
              for (k = zstart; k <= zend; k++)
                for (j = ystart; j <= yend; j++)
                  for (i = xstart; i <= xend; i++) {
                    ...
                    pDiagArray[K][J][Ii] =
                        pDiagArray[K][J][Ii] + nxVpsloc * nyVpsloc * nzVpsloc;
                    pOffDiagArray[K][J][Ii] = pOffDiagArray[K][J][Ii] +
                                             (nxVps * nyVps * nzVps -
                                              nxVpsloc * nyVpsloc * nzVpsloc);
                  }
            }
          }
    }
  ...
  for (k = 0; k < lzdim; k++)
    for (j = 0; j < lydim; j++)
      for (i = 0; i < lxdim; i++) {
        ...
        if (pSddft->nnzDArray[ctr] + pSddft->nnzODArray[ctr] >
            n_x * n_y * n_z) {
          pSddft->nnzDArray[ctr] = n_x * n_y * n_z / comm_size;
          pSddft->nnzODArray[ctr] =
              (comm_size - 1) * n_x * n_y * n_z / comm_size;
        }
        ctr++;
      }
