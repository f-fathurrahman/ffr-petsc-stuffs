/*=============================================================================================
  ...
  nonlocal | pseudopotential part of the Hamiltonian matrix for k-point sampling
  code
  |
...
///////////////////////////////////////////////////////////////////////////////////////////////
//            kPointHamiltonian_MatCreate: creates -(1/2)laplacian+Vnonloc
//            operator          //
...
PetscErrorCode kPointHamiltonian_MatCreate(SDDFT_OBJ *pSddft) {
  ...
  /*
   * create gamma point hamiltonian first to get the exact non-zero positions of
   * the hamiltonian
   */
  PeriodicLaplacianNonlocalPseudopotential_MatInit(pSddft);
  /*
   * duplicate the gamma point Hamiltonian to create the real Hamiltonian matrix
   * for k-point
   */
  MatDuplicate(pSddft->HamiltonianOpr, MAT_SHARE_NONZERO_PATTERN,
               &pSddft->HamiltonianOpr1);
  /*
   * duplicate the gamma point Hamiltonian to create the imaginary Hamiltonian
   * matrix for k-point
   */
  MatDuplicate(pSddft->HamiltonianOpr, MAT_SHARE_NONZERO_PATTERN,
               &pSddft->HamiltonianOpr2);
  /*
...
//  operator       //
///////////////////////////////////////////////////////////////////////////////////////////////
PetscErrorCode kPointHamiltonian_MatInit(SDDFT_OBJ *pSddft, PetscScalar k1,
                                         PetscScalar k2, PetscScalar k3) {
  /*
   * for a given k-point (k1,k2,k3), put the entries of HamiltonianOpr1 and
   * HamiltonianOpr2 1) Real: HamiltonianOpr1 = -0.5*Laplacian operator + Real
   * part of Nonlocal operator 2) Imag: HamiltonianOpr2 = -k.Gradient operator +
   * Imaginary part of Nonlocal operator
   */
  ...
  PetscInt xs, ys, zs, xl, yl, zl, xstart = -1, ystart = -1, zstart = -1,
                                   ...
                                   colidx, colidx_real, colidx_imag;
  PetscScalar x0, y0, z0, X0, Y0, Z0, cutoffr, max1, max2, max, X, Y, Z, XX, YY,
      ZZ;
  ...
  PetscScalar **YDU;
  PetscErrorCode ierr;
  PetscScalar SHreal_row, SHimag_row, SHreal_col, SHimag_col, UlDelVl, Ul,
      DUlDelVl, DUl;

  PetscScalar PlmJ_row, PlmJ_col, QlmJ_row, QlmJ_col, Tlmk_row, Tlmk_col,
      Slmk_row, Slmk_col;
  PetscScalar Rcut;
  PetscScalar dr = 1e-3;
  int OrbitalSize;
  ...
  PetscInt nzVps, nyVps, nxVps, nzVpsloc, nyVpsloc, nxVpsloc, i0, j0, k0,
      poscnt, index = 0, at, ii, jj, kk, l, m;
  ...
  for (k = zcor; k < zcor + lzdim; k++)
    for (j = ycor; j < ycor + lydim; j++)
      for (i = xcor; i < xcor + lxdim; i++) {
        ...
        MatSetValuesStencil(pSddft->HamiltonianOpr1, 1, &Laprow, 6 * o + 1,
                            Lapcol, Lapval, ADD_VALUES);
      }

  ierr = MatAssemblyBegin(pSddft->HamiltonianOpr1, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(pSddft->HamiltonianOpr1, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);
  ...
  MatAXPY(pSddft->HamiltonianOpr2, -k1, pSddft->gradient_x,
          SUBSET_NONZERO_PATTERN);
  MatAXPY(pSddft->HamiltonianOpr2, -k2, pSddft->gradient_y,
          SUBSET_NONZERO_PATTERN);
  MatAXPY(pSddft->HamiltonianOpr2, -k3, pSddft->gradient_z,
          SUBSET_NONZERO_PATTERN);
    ...
    if (lmax != 0) {
      ...
      do {
        ...
        count++;
               ...
               PetscRealPart(1.732 * pSddft->CUTOFF[at] + 4.0));
      rmax = tableR[count - 1];
      ...
      for (l = 0; l <= lmax; l++) {
        ...
        if (l != lloc) {
          ...
          Dexact[l] = (Dexact[l] -
                      ...
                      (delta * delta * delta); // weight of last point is 0.5
        }
      }
          ...
          for (QQ = Imin_y; QQ <= Imax_y; QQ++)
            for (RR = Imin_z; RR <= Imax_z; RR++) {
              ...
              if (overlap) {
                ...
                PetscMalloc(sizeof(PetscScalar) * (nzVps * nyVps * nxVps),
                            &val_imag);
                PetscMalloc(sizeof(PetscInt) * (nzVps * nyVps * nxVps),
                            &LIcol_real);
                PetscMalloc(sizeof(PetscInt) * (nzVps * nyVps * nxVps),
                            &LIcol_imag);
                  ...
                  for (j = ystart; j <= yend; j++)
                    for (i = xstart; i <= xend; i++) {
                      ...
                      for (kk = zs; kk <= zl; kk++)
                        for (jj = ys; jj <= yl; jj++)
                          for (ii = xs; ii <= xl; ii++) {
                              ...
                              if (l != lloc) {
                                ...
                                for (m = -l; m <= l; m++) {
                                  ComplexSphericalHarmonic(
                                      ...
                                      &SHreal_row, &SHimag_row);
                                  ComplexSphericalHarmonic(
                                      xx - x0, yy - y0, zz - z0, l, m, Rcut,
                                      &SHreal_col, &SHimag_col);
                                  ...
                                  Tlmk_row =
                                      cos(k1 * X + k2 * Y + k3 * Z) * PlmJ_row +
                                      sin(k1 * X + k2 * Y + k3 * Z) * QlmJ_row;
                                  Slmk_row =
                                      cos(k1 * X + k2 * Y + k3 * Z) * QlmJ_row -
                                      sin(k1 * X + k2 * Y + k3 * Z) * PlmJ_row;

                                  Tlmk_col = cos(k1 * XX + k2 * YY + k3 * ZZ) *
                                                 PlmJ_col +
                                             sin(k1 * XX + k2 * YY + k3 * ZZ) *
                                                 QlmJ_col;
                                  Slmk_col = cos(k1 * XX + k2 * YY + k3 * ZZ) *
                                                 QlmJ_col -
                                             sin(k1 * XX + k2 * YY + k3 * ZZ) *
                                                 PlmJ_col;

                                  val_real[colidx_real] +=
                                      (Tlmk_row * Tlmk_col +
                                       Slmk_row * Slmk_col) /
                                      Dexact[l];
                                  val_imag[colidx_imag] +=
                                      (Slmk_row * Tlmk_col -
                                       Tlmk_row * Slmk_col) /
                                      Dexact[l];
                                }
                            ...
                            /*
                             * map nodes from outside the computational domain
                             * to inside the computational domain
                             */
                             ...
                             * was employed
                             */
                            LIcol_real[colidx_real] =
                                roundf((ZZ + R_z) / delta) * n_x * n_y +
                                roundf((YY + R_y) / delta) * n_x +
                                roundf((XX + R_x) / delta);
                            LIcol_imag[colidx_imag] =
                                roundf((ZZ + R_z) / delta) * n_x * n_y +
                                roundf((YY + R_y) / delta) * n_x +
                                roundf((XX + R_x) / delta);

                            /*
                             * only values which are greater than 1e-16 are
                             * taken
                             */
                      ...
                      if (colidx_real > 0) {
                        ...
                        ierr = MatSetValues(pSddft->HamiltonianOpr1, 1, &LIrow,
                                            colidx_real, LIcol_real, val_real,
                                            ADD_VALUES);
                        CHKERRQ(ierr);
                      }
                      if (colidx_imag > 0) {
                        AOApplicationToPetsc(aodmda, colidx_imag, LIcol_imag);
                        ierr = MatSetValues(pSddft->HamiltonianOpr2, 1, &LIrow,
                                            colidx_imag, LIcol_imag, val_imag,
                                            ADD_VALUES);
                        CHKERRQ(ierr);
                      }
                    }
   ...
   * assemble matrix
   */
  ierr = MatAssemblyBegin(pSddft->HamiltonianOpr1, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(pSddft->HamiltonianOpr1, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);

  ierr = MatAssemblyBegin(pSddft->HamiltonianOpr2, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(pSddft->HamiltonianOpr2, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
